...
#define MSSW_VERSION      "3.6B"
#define MSSW_VERSION_DATE "8-Mar-97"
...
DXDEC  void FAR * WINAPI AIL_mem_alloc_lock(U32       size);
DXDEC  void       WINAPI AIL_mem_free_lock (void FAR *ptr);

DXDEC  S32        WINAPI AIL_file_error   (void);

DXDEC  S32        WINAPI AIL_file_size    (char FAR   *filename);

DXDEC  void FAR * WINAPI AIL_file_read    (char FAR   *filename, 
                                           void FAR *dest);

DXDEC  S32        WINAPI AIL_file_write   (char FAR   *filename, 
                                           void FAR *buf, 
                                           U32       len);

extern void FAR * AIL_ptr_add             (void FAR *ptr, 
                                           U32       offset);

extern S32        AIL_ptr_dif             (void FAR *p1,  
                                           void FAR *p2);

extern S32        AIL_strnicmp           (void FAR *s1,
                                           void FAR *s2,
                                           U32       count);

extern U8 FAR *   AIL_strstr             (void FAR *s1, 
                                           void FAR *s2);

extern void       AIL_memcpy             (void FAR *s1, 
                                           void FAR *s2, 
                                           U32       count);
...
DXDEC void    WINAPI AIL_quick_handles             ( HDIGDRIVER* pdig,
                                                     HMDIDRIVER* pmdi );

DXDEC HAUDIO  WINAPI AIL_quick_load                (char   FAR *filename);

DXDEC HAUDIO  WINAPI AIL_quick_copy                (HAUDIO      audio);
...
DXDEC void   WINAPI AIL_quick_set_speed (HAUDIO audio, S32 speed);

DXDEC void   WINAPI AIL_quick_set_volume (HAUDIO audio, S32 volume, S32 extravol);
...
DXDEC  S32          WINAPI AIL_digital_handle_release(HDIGDRIVER drvr);

DXDEC  S32          WINAPI AIL_digital_handle_reacquire
                                                     (HDIGDRIVER drvr);
...
DXDEC  S32          WINAPI AIL_MIDI_handle_release
                                                 (HMDIDRIVER mdi);

DXDEC  S32          WINAPI AIL_MIDI_handle_reacquire
                                                 (HMDIDRIVER mdi);
...
DXDEC  void       WINAPI AIL_redbook_retract(HREDBOOK hand);

DXDEC  U32        WINAPI AIL_redbook_status(HREDBOOK hand);

DXDEC  U32        WINAPI AIL_redbook_tracks(HREDBOOK hand);

DXDEC  U32        WINAPI AIL_redbook_track(HREDBOOK hand);
...
DXDEC  S32        WINAPI AIL_redbook_volume(HREDBOOK hand);

DXDEC  S32        WINAPI AIL_redbook_set_volume(HREDBOOK hand, S32 volume);

#ifdef IS_WIN32
  DXDEC U32 WINAPI AIL_ms_count();
#else
...
typedef struct _STREAM {
  ...
  U32 size2;       // next buffer loaded up to

  U32 buf3;        // 0,1,2 (the next buffer that we are reading into)
  U32 size3;       // next buffer loaded up to
...
DXDEC HSTREAM WINAPI AIL_open_stream(HDIGDRIVER dig, char FAR* filename, S32 stream_mem);

DXDEC void WINAPI AIL_close_stream(HSTREAM stream);

DXDEC S32 WINAPI AIL_service_stream(HSTREAM stream, S32 fillup);

DXDEC void WINAPI AIL_start_stream(HSTREAM stream);

DXDEC void WINAPI AIL_pause_stream(HSTREAM stream, S32 onoff);

DXDEC void WINAPI AIL_set_stream_volume(HSTREAM stream,S32 volume);

DXDEC void WINAPI AIL_set_stream_pan(HSTREAM stream,S32 pan);

DXDEC S32 WINAPI AIL_stream_volume(HSTREAM stream);

DXDEC S32 WINAPI AIL_stream_pan(HSTREAM stream);

DXDEC void WINAPI AIL_set_stream_playback_rate(HSTREAM stream, S32 rate);

DXDEC S32 WINAPI AIL_stream_playback_rate(HSTREAM stream);

DXDEC S32 WINAPI AIL_stream_loop_count(HSTREAM stream);

DXDEC void WINAPI AIL_set_stream_loop_count(HSTREAM stream, S32 count);

DXDEC S32 WINAPI AIL_stream_status(HSTREAM stream);

DXDEC void WINAPI AIL_set_stream_position(HSTREAM stream,S32 offset);

DXDEC S32 WINAPI AIL_stream_position(HSTREAM stream);

DXDEC void WINAPI AIL_stream_info(HSTREAM stream, S32 FAR* datarate, S32 FAR* sndtype, S32 FAR* length, S32 FAR* memory);

DXDEC AILSTREAMCB WINAPI AIL_register_stream_callback(HSTREAM stream, AILSTREAMCB callback);

DXDEC void WINAPI AIL_auto_service_stream(HSTREAM stream, S32 onoff);

DXDEC  void     WINAPI AIL_set_stream_user_data   (HSAMPLE S,
                                                     U32     index,
                                                     S32     value);

DXDEC  S32      WINAPI AIL_stream_user_data       (HSAMPLE S,
                                                     U32     index);
