...
class CBridgeHex : public CMap<DWORD, DWORD, CBridgeUnit*, CBridgeUnit*>
{
  public:
    ...
    CBridgeUnit* GetBridge( CHexCoord const& hex ) const { return ( GetBridge( hex.X( ), hex.Y( ) ) ); }
    CBridgeUnit* GetBridge( CSubHex const& sub ) const { return ( GetBridge( sub.x / 2, sub.y / 2 ) ); }
    CBridgeUnit* GetBridge( int x, int y ) const
    {
        CBridgeUnit* pBrdg;
        if ( Lookup( ToArg( x, y ), pBrdg ) )
            return ( pBrdg );
        return ( NULL );
    }

    CBridgeUnit* _GetBridge( CHexCoord const& hex ) const { return ( _GetBridge( hex.X( ), hex.Y( ) ) ); }
    CBridgeUnit* _GetBridge( CSubHex const& sub ) const { return ( _GetBridge( sub.x / 2, sub.y / 2 ) ); }
    CBridgeUnit* _GetBridge( int x, int y ) const
    {
        CBridgeUnit* pBrdg;
        if ( Lookup( _ToArg( x, y ), pBrdg ) )
            return ( pBrdg );
        return ( NULL );
    }
  ...
  protected:
    ...
    DWORD _ToArg( int x, int y ) const { return ( ( (DWORD)x << 16 ) | (DWORD)y ); }
};
