...
//
static int aiCbtPath[CRsrchArray::num_types] = { CRsrchArray::fire_control,
                                                 CRsrchArray::armored_vehicle,
                                                 CRsrchArray::medium_vehicle,
                                                 ...
                                                 CRsrchArray::advanced_facilities,
                                                 CRsrchArray::balloons,
                                                 CRsrchArray::gliders,
                                                 CRsrchArray::prop_planes,
                                                 CRsrchArray::jet_planes,
                                                 CRsrchArray::rockets,
                                                 CRsrchArray::atk_1,
                                                 CRsrchArray::def_1,
                                                 CRsrchArray::range_1,
                                                 ...
                                                 CRsrchArray::def_3,
                                                 CRsrchArray::acc_3,
                                                 CRsrchArray::sailboats,
                                                 CRsrchArray::motorboats,
                                                 CRsrchArray::heavy_naval,
                                                 CRsrchArray::spot_3,
                                                 CRsrchArray::bridge,
                                                 CRsrchArray::farm_1,
                                                 CRsrchArray::mine_1,
...
static int aiEricPath[CRsrchArray::num_types] = { CRsrchArray::armored_vehicle,
                                                  CRsrchArray::tanks,
                                                  CRsrchArray::fire_control,
                                                  CRsrchArray::medium_facilities,
                                                  CRsrchArray::medium_vehicle,
                                                  CRsrchArray::gas_turbine,
                                                  CRsrchArray::artillery,
                                                  ...
                                                  CRsrchArray::mail,
                                                  CRsrchArray::email,
                                                  CRsrchArray::telephone };


// very naval oriented
static int aiStevePath[CRsrchArray::num_types] = { CRsrchArray::armored_vehicle,
                                                   CRsrchArray::tanks,
                                                   CRsrchArray::medium_facilities,
                                                   CRsrchArray::copper,
                                                   CRsrchArray::radio,
...
static int aiKeithPath[CRsrchArray::num_types] = { CRsrchArray::fire_control,
                                                   CRsrchArray::medium_facilities,
                                                   CRsrchArray::armored_vehicle,
                                                   CRsrchArray::tanks,
                                                   CRsrchArray::gas_turbine,
...
CAIGoalMgr::CAIGoalMgr( BOOL bRestart, int iPlayer, CAIMap* pMap, CAIUnitList* plUnits, CAIOpForList* plOpFors )
{
    ...
    m_iPowerLvl = 1;
    for ( int i = 0; i < CRsrchArray::num_types; ++i ) m_iRDPath[i] = 0;
...
void CAIGoalMgr::Assess( CAIMsg* pMsg )
{
    ...
    if ( pMsg->m_iMsg == CNetCmd::bldg_new && pMsg->m_idata3 != m_iPlayer && pMsg->m_idata1 == CStructureData::rocket )
    {
        ...
        if ( m_pwaAttribs[CRaceDef::attack] >= 90 && pGameData->m_iSmart )
        {
    ...
    if ( pMsg->m_iMsg == CNetCmd::cmd_play )
    {
        ...
        logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "m_iLand/m_iOcean/m_iLake %d/%d/%d \n", m_pMap->m_iLand,
                   m_pMap->m_iOcean, m_pMap->m_iLake );
#endif

        if ( ( m_bOceanWorld || m_bLakeWorld ) && pGameData->m_iSmart )
        {
    ...
    if ( ( pMsg->m_iMsg == CNetCmd::veh_new || pMsg->m_iMsg == CNetCmd::delete_unit ||
    ...
    if ( m_bProductionChange || m_bGoalChange || m_bMapChanged )
...
void CAIGoalMgr::ConsiderThreats( CAIMsg* pMsg )
{
...
#ifdef _LOGOUT
    logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "CAIGoalMgr::ConsiderThreats() for %d executing ", m_iPlayer );
#endif
    ...
    if ( pMsg->m_iMsg == CNetCmd::see_unit )
    {
            ...
            while ( pos != NULL )
            {
                ...
                if ( paiOpFor != NULL )
                {
                    ...
                    if ( paiOpFor->GetOwner( ) != iOpForID )
                        continue;
                    if ( paiOpFor->GetTypeUnit( ) == pTarget->GetTypeUnit( ) )
                        wCnt++;
                }
            }
        ...
        if ( pTarget->GetType( ) == CUnit::building )
        {
            ...
            if ( pTarget->GetTypeUnit( ) == CStructureData::rocket && pGameData->m_iSmart )
            {
                pGoal = m_plGoalList->GetGoal( IDG_ADVDEFENSE );
                if ( pGoal == NULL )
                {
        ...
        else if ( pTarget->GetType( ) == CUnit::vehicle )
        {
            ...
            if ( pGoal == NULL )
            {
                if ( pOpFor->GetVehicle( CTransportData::gun_boat ) ||
                     pOpFor->GetVehicle( CTransportData::destroyer ) || pOpFor->GetVehicle( CTransportData::cruiser ) ||
            ...
            if ( pGoal == NULL )
            {
                if ( pOpFor->GetVehicle( CTransportData::light_cargo ) ||
                     pOpFor->GetVehicle( CTransportData::gun_boat ) || pOpFor->GetVehicle( CTransportData::destroyer ) )
                {
                    AddGoal( IDG_PIRATE );
                    m_bGoalChange = TRUE;
                }
            ...
            if ( pTask != NULL )
            {
                     ...
                     pTask->GetTaskParam( PRODUCTION_TYPE ) == CStructureData::UTvehicle )
                {
                    ...
                    if ( m_pwaVehGoals[iVeh] )
                    {
                        ...
                        {
                            WORD wQty = pTask->GetTaskParam( PRODUCTION_QTY ) + 1;
...
void CAIGoalMgr::AttackPlayer( int iOpforID )
{
    ...
    if ( m_bOceanWorld )
    {
        ...
        if ( pOpFor->GetVehicle( CTransportData::gun_boat ) || pOpFor->GetVehicle( CTransportData::destroyer ) ||
             pOpFor->GetVehicle( CTransportData::cruiser ) || pOpFor->GetVehicle( CTransportData::landing_craft ) )
        {
            pGoal = m_plGoalList->GetGoal( IDG_REPELL );
            if ( pGoal == NULL )
            {
    ...
    while ( posU != NULL )
    {
        ...
        if ( pVeh != NULL )
        {
            ...
            dwTarget = GetOpforTarget( pOpFor->GetPlayerID( ), pVeh->GetTypeUnit( ) );
            if ( dwTarget )
            {
                ...
                if ( hexVeh.X( ) || hexVeh.Y( ) )
                {
                    ...
                    m_pMap->m_pMapUtil->FindStagingHex( hexVeh, 1, 1, pVeh->GetTypeUnit( ), hexDest );
...
#ifdef _LOGOUT
    logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "CAIGoalMgr::player %d scenario building goal %d task %d priority %d ",
...
void CAIGoalMgr::ConsiderProduction( CAIMsg* pMsg )
{
    ...
    if ( pMsg->m_iMsg == CNetCmd::bldg_new && pMsg->m_idata1 == CStructureData::rocket )
        m_dwRocket = pMsg->m_dwID;
    // start vehicles must add to goals
    if ( pMsg->m_iMsg == CNetCmd::veh_new && pMsg->m_dwID2 == m_dwRocket )
...
#ifdef _LOGOUT
    logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "CAIGoalMgr::ConsiderProduction() for %d executing ", m_iPlayer );
#endif
    ...
    */
    ...
    {
        if ( ( m_pwaVehGoals[i] > m_pwaUnits[i] ) && i == iVeh )
            ProduceVehicles( i );
    }
}
...
void CAIGoalMgr::ConsiderTrucks( CAIMsg* pMsg )
{
    ...
    logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "\nCAIGoalMgr::ConsiderTrucks() player %d task=%d priority=%d qty=%d \n",
               m_iPlayer, pTask->GetID( ), (int)pTask->GetPriority( ), pTask->GetTaskParam( PRODUCTION_QTY ) );
#endif
}
...
BOOL CAIGoalMgr::NeedCranes( void )
{
    ...
    while ( pos != NULL )
    {
        ...
        if ( pUnit != NULL )
        {
            ...
            if ( pUnit->GetTypeUnit( ) == CStructureData::light_0 || pUnit->GetTypeUnit( ) == CStructureData::light_1 ||
                 pUnit->GetTypeUnit( ) == CStructureData::light_2 || pUnit->GetTypeUnit( ) == CStructureData::heavy )
            {
...
void CAIGoalMgr::NeedTrucks( void )
{
    ...
    while ( pos != NULL )
    {
        ...
        if ( pUnit != NULL )
        {
            ...
            {
                if ( pUnit->GetTask( ) != IDT_MAKEHTRUCK && pUnit->GetTask( ) != IDT_MAKEMTRUCK )
                    bMakeTrucks = TRUE;
            }
        }
    }
    ...
    CAITask* pStdTask = plTaskList->GetTask( pTask->GetID( ), pTask->GetGoalID( ) );
    if ( pStdTask == NULL )
    {
...
BOOL CAIGoalMgr::NeedGas( void )
{
    ...
    if ( !m_iGasHave )
    {
        ...
        if ( m_pwaBldgs[CStructureData::refinery] == m_pwaBldgGoals[CStructureData::refinery] &&
             m_pwaBldgs[CStructureData::oil_well] == m_pwaBldgGoals[CStructureData::oil_well] )
...
void CAIGoalMgr::ConsiderRoads( void )
{
    ...
    CAITask* pRoadTask = m_plTasks->GetTask( IDT_CONSTRUCT, IDG_ESTABLISH );
    if ( pRoadTask == NULL )
        return;
...
#ifdef _LOGOUT
        logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "\nCAIGoalMgr::ConsiderRoads() player %d out of roads %d", m_iPlayer,
                   m_pMap->m_iRoadCount );
#endif
        return;
    }
...
"\nCAIGoalMgr::ConsiderRoads() player %d road priority completed", m_iPlayer );
    ...
    {
#ifdef _LOGOUT
        logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "CAIGoalMgr::ConsiderRoads() player %d road priority already set high",
                   m_iPlayer );
#endif
        return;
    }
...
void CAIGoalMgr::IdleCrane( void )
{
    ...
    if ( m_pwaBldgs[CStructureData::research] >= m_pwaBldgGoals[CStructureData::research] )
    {
...
BOOL CAIGoalMgr::CheckAbandonedBuildings( void )
{
    ...
    for ( int iBldg = CStructureData::lumber; iBldg <= CStructureData::copper; ++iBldg )
    {
...
void CAIGoalMgr::CheckPlayer( void )
{
    ...
    if ( m_iGasHave < m_iGasNeed )
    {
        ...
        if ( m_pwaBldgs[CStructureData::oil_well] )
        {
            ...
            if ( m_pwaBldgs[CStructureData::refinery] == m_pwaBldgGoals[CStructureData::refinery] )
            {
                if ( m_pwaBldgs[CStructureData::refinery] < pGameData->m_iSmart &&
                     m_pwaBldgs[CStructureData::refinery] <= ( ( m_pwaBldgGoals[CStructureData::oil_well] + 1 ) / 2 ) )
                {
...
void CAIGoalMgr::AddPowerTask( )
{
    ...
    if ( !m_pwaBldgs[CStructureData::power_1] || !m_pwaBldgs[CStructureData::oil_well] ||
         !m_pwaBldgs[CStructureData::refinery] || !m_pwaBldgs[CStructureData::lumber] ||
         !m_pwaBldgs[CStructureData::farm] || !m_pwaBldgs[CStructureData::research] ||
         !m_pwaBldgs[CStructureData::smelter] || !m_pwaBldgs[CStructureData::coal] ||
         !m_pwaBldgs[CStructureData::iron] )
        return;


#ifdef _LOGOUT
    logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "CAIGoalMgr::AddPowerTask() for %d ", m_iPlayer );
#endif
...
void CAIGoalMgr::ProduceMaterials( int iMat )
{
    ...
    else
    {
        ...
        logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "onhand=%d  need=%d  added goal %d ", m_pwaMatOnHand[iMat],
                   m_pwaMatGoals[iMat], wGoal );
        logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "bldgs=%d  need=%d \n", m_pwaBldgs[iBldg], m_pwaBldgGoals[iBldg] );
#endif
    }
    m_bGoalChange = TRUE;
...
BOOL CAIGoalMgr::AddNewProducer( int iBldg, int iMat )
{
    ...
    if ( iMat != CMaterialTypes::food )
    {
        if ( m_pwaMatGoals[iMat] < ( m_pwaMatOnHand[iMat] * 2 ) )
            return ( FALSE );
    }
    ...
    while ( pos != NULL )
    {
        ...
        if ( pUnit != NULL )
        {
            ...
            if ( pUnit->GetType( ) == CUnit::building && pUnit->GetTypeUnit( ) == iBldg )
            {
    ...
    if ( iBldg == CStructureData::coal || iBldg == CStructureData::iron )
    {
    ...
    if ( iBldg == CStructureData::smelter )
    {
        if ( !m_pwaBldgs[CStructureData::power_1] || !m_pwaBldgs[CStructureData::oil_well] ||
             !m_pwaBldgs[CStructureData::refinery] || !m_pwaBldgs[CStructureData::lumber] ||
             !m_pwaBldgs[CStructureData::farm] || !m_pwaBldgs[CStructureData::research] ||
             !m_pwaBldgs[CStructureData::coal] || !m_pwaBldgs[CStructureData::iron] )
            return ( FALSE );
    }
        ...
        if ( pTask != NULL )
        {
            ...
            if ( iBldg == pTask->GetTaskParam( BUILDING_ID ) )
            {
                if ( pTask->GetStatus( ) == UNASSIGNED_TASK )
                {
                    ...
                    {
                        // get standard task data
                        CAITask* pStdTask = plTaskList->GetTask( pTask->GetID( ), 0 );
                        if ( pStdTask == NULL )
                            continue;
...
#ifdef _LOGOUT
                        logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC,
                                   "CAIGoalMgr::AddNewProducer() player %d for a %d building, goal %d task %d reset "
                                   "priority %d ",
                                   m_iPlayer, iBldg, pTask->GetGoalID( ), pTask->GetID( ), pTask->GetPriority( ) );
                        logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "CAIGoalMgr::material %d  on-hand %d  needed %d ", iMat,
                                   m_pwaMatOnHand[iMat], m_pwaMatGoals[iMat] );
#endif
                ...
                if ( pTask->GetTaskParam( BUILD_AT_X ) || pTask->GetTaskParam( BUILD_AT_Y ) )
                    return ( FALSE );

                // if have less buildings built, than the task requests
                // then there is no reason to add more to the task
                if ( m_pwaBldgs[iBldg] < pTask->GetTaskParam( PRODUCTION_QTY ) )
                    return ( FALSE );
...
void CAIGoalMgr::ProduceVehicles( int iVeh )
{
...
#ifdef _LOGOUT
        logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "\nCAIGoalMgr::ProduceVehicles() player %d for a %d ", m_iPlayer,
                   iVeh );
        logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "vehicles=%d  need=%d  increased task %d to quantity %d ",
                   m_pwaUnits[iVeh], m_pwaVehGoals[iVeh], wTask, iQty );
        logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "factories=%d  need=%d ", m_pwaBldgs[iBldg1], m_pwaBldgGoals[iBldg1] );
        if ( iBldg2 < m_iNumBldgs )
            logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "factories=%d  need=%d ", m_pwaBldgs[iBldg2],
    ...
    else
    {
        ...
        logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "vehicles=%d  need=%d  added goal %d \n", m_pwaUnits[iVeh],
                   m_pwaVehGoals[iVeh], wGoal );
#endif
    }
...
// types of factories
//
BOOL CAIGoalMgr::AddNewFactory( int iBldg1, int iBldg2, WORD& wTask, WORD& wGoal )
{
    // don't add if count of buildings < building goals
    if ( iBldg1 < CStructureData::num_types )
    {
        if ( m_pwaBldgs[iBldg1] < m_pwaBldgGoals[iBldg1] )
            return ( FALSE );
    }
    if ( iBldg2 < CStructureData::num_types )
    {
        if ( m_pwaBldgs[iBldg2] < m_pwaBldgGoals[iBldg2] )
            return ( FALSE );
    }
    ...
    while ( pos != NULL )
    {
        ...
        if ( pUnit != NULL )
        {
            ...
            if ( pUnit->GetType( ) == CUnit::building &&
                 ( pUnit->GetTypeUnit( ) == iBldg1 || pUnit->GetTypeUnit( ) == iBldg2 ) )
            {
                BOOL bConstructing = FALSE;
                BOOL bWaiting      = FALSE;
                EnterCriticalSection( &cs );
    ...
    while ( pos != NULL )
    {
        ...
        if ( pTask != NULL )
        {
            ...
            if ( iBldg1 == pTask->GetTaskParam( BUILDING_ID ) || iBldg2 == pTask->GetTaskParam( BUILDING_ID ) )
            {
    ...
    if ( pMsg->m_idata1 < m_iNumUnits )
    {
        ...
        while ( pos != NULL )
        {
            ...
            if ( pTask != NULL )
            {
                ...
                if ( pTask->GetTaskParam( ORDER_TYPE ) == PRODUCTION_ORDER &&
                     pTask->GetTaskParam( PRODUCTION_TYPE ) == CStructureData::UTvehicle &&
                     (int)pTask->GetTaskParam( PRODUCTION_ITEM ) == pMsg->m_idata1 )
                {
#if THREADS_ENABLED
                    // BUGBUG this function must yield
                    myYieldThread( );
                    ...
                    //	throw(ERR_CAI_TM_QUIT); // THROW( pException );
#endif
                    int iCnt = (int)pTask->GetTaskParam( PRODUCTION_QTY ) + 1;
                    pTask->SetTaskParam( PRODUCTION_QTY, (WORD)iCnt );

#ifdef _LOGOUT
                    logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "CAIGoalMgr::SetStartVehicles for player %d ", m_iPlayer );
                    logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "task %d goal %d producing %d quantity %d ",
                               pTask->GetID( ), pTask->GetGoalID( ), pTask->GetTaskParam( PRODUCTION_ITEM ),
                               pTask->GetTaskParam( PRODUCTION_QTY ) );
#endif
...
int CAIGoalMgr::GetPowerToBe( )
{
    ...
    while ( pos != NULL )
    {
        ...
        if ( pTask != NULL )
        {
            ...
            // IDT_BUILDPOWERPT2
            // IDT_BUILDPOWERPT3
            if ( pTask->GetID( ) == IDT_BUILDPOWERPT1 || pTask->GetID( ) == IDT_BUILDPOWERPT2 ||
...
void CAIGoalMgr::UpdateVehGoals( void )
{
    ...
    while ( pos != NULL )
    {
        ...
        if ( pTask != NULL )
        {
            ...
            else if ( pTask->GetTaskParam( ORDER_TYPE ) == CONSTRUCTION_ORDER &&
    ...
    m_pwaBldgGoals[CStructureData::fort_1] += ( iCnt + pGameData->GetRandom( iCnt ) );
    m_pwaBldgGoals[CStructureData::fort_2] += ( iCnt + pGameData->GetRandom( iRatio ) );
    ...
    for ( int i = 0; i < m_iNumUnits; ++i )
    {
        ...
        if ( pGameData->IsCombatVehicle( i ) )
        {
            ...
            if ( i == CTransportData::infantry_carrier || i == CTransportData::infantry ||
...
void CAIGoalMgr::GetCommodityNeeds( void )
{
    ...
    while ( pos != NULL )
    {
        ...
        if ( pUnit != NULL )
        {
            ...
            if ( pUnit->GetType( ) == CUnit::building )
            {
                ...
                if ( pCopyCBuilding->m_aiDataOut[CAI_ISCONSTRUCTING] )
                {
                    ...
                    CAICopy* pCopyCStructureData = pUnit->GetCopyData( CAICopy::CStructureData );
       ...
       //  BUGBUG this function must yield
							...
							//	throw(ERR_CAI_TM_QUIT); // THROW( pException );
#endif
                            WORD wCnt        = m_pwaMatGoals[i] + (WORD)pCopyCStructureData->m_aiDataIn[i];
...
#endif
                        // add the materials needed to material goals
                        WORD wCnt        = m_pwaMatGoals[i] + (WORD)pCopyCBuildMaterials->m_aiDataIn[i];
                        m_pwaMatGoals[i] = wCnt;

                        // add the materials produced to on-hand
                        wCnt              = m_pwaMatOnHand[i] + (WORD)pCopyCBuildMaterials->m_aiDataOut[i];
       ...
       //  BUGBUG this function must yield
                        ...
                        WORD wCnt        = m_pwaMatGoals[i] + (WORD)pCopyCBuildUnit->m_aiDataIn[i];
                        m_pwaMatGoals[i] = wCnt;
                    }
                }
                else if ( pCopyCBuilding->m_aiDataOut[CAI_PRODUCES] == CStructureData::UTpower )
                {
                    ...
                    if ( pBldgData != NULL )
                    {
                        ...
                        if ( pPowerBldg->GetInput( ) >= 0 )
                        {
                            ...
                            m_pwaMatGoals[i] += (int)( ( 86400L / (LONG)pPowerBldg->GetRate( ) ) / 24 );
                ...
                for ( int i = 0; i < m_iNumMats; ++i )
                {
                    WORD wCnt         = m_pwaMatOnHand[i] + (WORD)pCopyCBuilding->m_aiDataIn[i];
                    m_pwaMatOnHand[i] = wCnt;
                }
            }
            else if ( pUnit->GetType( ) == CUnit::vehicle )
            {
                ...
                for ( int i = 0; i < m_iNumMats; ++i )
                {
                    WORD wCnt         = m_pwaMatOnHand[i] + (WORD)pCopyCVehicle->m_aiDataIn[i];
                    m_pwaMatOnHand[i] = wCnt;
                }
            }
        }
    ...
    if ( m_pwaMatGoals[CMaterialTypes::food] > m_pwaMatOnHand[CMaterialTypes::food] )
    {
        ...
        if ( ( m_pwaMatOnHand[CMaterialTypes::food] * 120 ) < iFood )
        {
            ...
            m_pwaMatGoals[CMaterialTypes::food] = m_pwaMatOnHand[CMaterialTypes::food];
        }
        else
        {
            // increase the difference to encourage farm construction
            m_pwaMatGoals[CMaterialTypes::food] = m_pwaMatOnHand[CMaterialTypes::food] * 120;

#ifdef _LOGOUT
            logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "CAIGoalMgr::GetCommodityNeeds: need food for player %d ",
                       m_iPlayer );
#endif
        }
    }
...
WORD CAIGoalMgr::GetVehGoals( WORD wOffset )
{
    ...
    if ( m_pwaVehGoals != NULL )
    {
        ...
        {
            WORD wCnt = 0;
            for ( int i = 0; i < m_iNumUnits; ++i ) wCnt += m_pwaVehGoals[i];
            return ( wCnt );
        }
    }
    return ( FALSE );
...
void CAIGoalMgr::UpdateTaskPriorities( CAIMsg* pMsg )
{
    ...
    //
    if ( pMsg->m_iMsg == CNetCmd::bldg_stat && pMsg->m_idata3 == m_iPlayer )
    {
    ...
    if ( pMsg->m_iMsg == CNetCmd::road_done || pMsg->m_iMsg == CNetCmd::cmd_play )
    {
        UpdateConstructionTasks( pMsg );
        bRunConstructionUpdate = FALSE;
    }
    ...
    if ( pMsg->m_iMsg == CNetCmd::veh_new && pMsg->m_idata3 == m_iPlayer )
    {
        UpdateProductionTasks( pMsg );
        UpdateCombatTasks( pMsg );
        // UpdateInfoTasks(pMsg);
    ...
    if ( pMsg->m_iMsg == CNetCmd::scenario && pMsg->m_idata3 == m_iPlayer )
    {
        UpdateConstructionTasks( pMsg );
        bRunConstructionUpdate = FALSE;
    ...
    if ( pMsg->m_iMsg == CNetCmd::delete_unit && pMsg->m_idata3 == m_iPlayer )
    {
        ...
        while ( pos != NULL )
        {
            ...
            if ( pTask != NULL )
            {
                ...
                if ( pTask->GetID( ) != pUnit->GetTask( ) || pTask->GetGoalID( ) != pUnit->GetGoal( ) )
                    continue;
                ...
                else  // a task is still staging or about to stage
                {
                    ...
                    if ( !pTask->GetPriority( ) )
                    {
                        ...
                        if ( pStdTask != NULL )
                        {
                            ...
                            if ( pTask->GetGoalID( ) == IDG_LANDWAR || pTask->GetGoalID( ) == IDG_SEAINVADE )
...
#ifdef _LOGOUT
                logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "player %d goal %d task %d priority %d \n", m_iPlayer,
...
void CAIGoalMgr::UpdateConstructionTasks( CAIMsg* pMsg )
{
    ...
    if ( pMsg != NULL && pMsg->m_iMsg == CNetCmd::delete_unit )
    {
        // m_dwID contains that unit
        // get its pointer and update
        CAIUnit* pUnit = m_plUnits->GetUnit( pMsg->m_dwID );
    ...
    for ( int i = 0; i < CTransportData::num_types; ++i ) awTypes[i] = 0;
        ...
        if ( pTask != NULL )
        {
            ...
            if ( pTask->GetTaskParam( ORDER_TYPE ) == CONSTRUCTION_ORDER )
            {
                ...
                if ( pTask->GetOrderID( ) == CNetCmd::build_road )
                {
                    ...
                    if ( !pTask->GetPriority( ) && m_pMap->m_iRoadCount )
                    {
                        // reset to standard priority
                        CAITask* pStdTask = plTaskList->FindTask( pTask->GetID( ) );
                        if ( pStdTask != NULL )
                            pTask->SetPriority( pStdTask->GetPriority( ) );
                    }
                    continue;
                ...
                if ( pTask->GetOrderID( ) == CNetCmd::repair_bldg || pTask->GetOrderID( ) == CNetCmd::repair_veh )
                {
                    if ( !pTask->GetPriority( ) )
                    {
                        // reset to standard priority
                ...
                if ( pTask->GetStatus( ) == UNASSIGNED_TASK )
                {
                    ...
                    if ( pBldgData->GetUnionType( ) == CStructureData::UThousing && bCanBuild )
                    {
                        // but, there may not be artwork for them,
                        if ( pTask->GetID( ) == IDT_BUILDAPTS || pTask->GetID( ) == IDT_BUILDOFFICE )
                        {
...
#ifdef _LOGOUT
                        logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC,
                                   "\nCAIGoalMgr::player %d goal %d task %d has not discovered ", m_iPlayer,
                                   pTask->GetGoalID( ), pTask->GetID( ) );
                        logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "building type %d with count=%d and goals=%d ", iBldg,
                                   m_pwaBldgs[iBldg], m_pwaBldgGoals[iBldg] );
#endif
                        continue;
                    }
...
#ifdef _LOGOUT
                    logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC,
                               "\nCAIGoalMgr::player %d goal %d task %d current priority %d ", m_iPlayer,
                               pTask->GetGoalID( ), pTask->GetID( ), pTask->GetPriority( ) );
                    logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "for building type %d, with count=%d and goals=%d ", iBldg,
                               m_pwaBldgs[iBldg], m_pwaBldgGoals[iBldg] );
#endif

                    // is there at least one of this type of building?
                    if ( m_pwaBldgs[iBldg] || !pTask->GetPriority( ) )
                    {
                        if ( m_pwaBldgs[iBldg] >= m_pwaBldgGoals[iBldg] )
                        {
                    ...
                    if ( !m_pwaBldgs[iBldg] )
                    {
                        ...
                        if ( iBldg == CStructureData::barracks_2 && pGameData->m_iSmart )
                        {
                            if ( m_pwaBldgs[CStructureData::smelter] && m_pwaBldgs[CStructureData::coal] &&
                                 m_pwaBldgs[CStructureData::iron] && !m_pwaUnits[CTransportData::infantry] )
                            {
                                if ( pStdTask != NULL )
                                    pTask->SetPriority( ( pStdTask->GetPriority( ) * 2 ) );
                            }
                    ...
                    if ( bPowerOnly && iBldg != CStructureData::farm && iBldg != CStructureData::smelter )
                    {
                        if ( pBldgData->GetUnionType( ) != CStructureData::UTpower &&
                             pBldgData->GetUnionType( ) != CStructureData::UTmine )
                        {
                        ...
                        else
                        {
                            ...
                            if ( iBldg == CStructureData::power_1 || iBldg == CStructureData::power_2 ||
                                 iBldg == CStructureData::power_3 )
                            {
                                if ( m_pwaBldgs[iBldg] && m_pwaBldgs[iBldg] >= m_pwaBldgGoals[iBldg] )
                                    pTask->SetPriority( (BYTE)0 );
                            }
                        }
...
#ifdef _LOGOUT
                        logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC,
                                   "CAIGoalMgr::player %d reset power/fuel goal %d task %d priority %d ", m_iPlayer,
                                   pTask->GetGoalID( ), pTask->GetID( ), pTask->GetPriority( ) );
#endif
                    }
                    ...
                    if ( pBldgData->GetUnionType( ) == CStructureData::UTvehicle ||
                         pBldgData->GetUnionType( ) == CStructureData::UTshipyard )
                    {
                        ...
                        if ( !m_pwaBldgs[iBldg] )
                        {
                            ...
                            if ( pBldgData->GetUnionType( ) == CStructureData::UTvehicle )
                            {
                                CBuildVehicle* pBldVeh = pBldgData->GetBldVehicle( );
                                if ( pBldVeh != NULL )
                                {
                                    for ( int i = 0; i < pBldVeh->GetSize( ); ++i )
                                    {
                                        CBuildUnit const* pBldUnit = pBldVeh->GetUnit( i );
                                        if ( pBldUnit != NULL && pBldUnit->GetVehType( ) < CTransportData::num_types )
                                        {
                            ...
                            else if ( pBldgData->GetUnionType( ) == CStructureData::UTshipyard )
                            {
                                CBuildShipyard* pBldShip = pBldgData->GetBldShipyard( );
                                if ( pBldShip != NULL )
                                {
                                    for ( int i = 0; i < pBldShip->GetSize( ); ++i )
                                    {
                                        CBuildUnit const* pBldUnit = pBldShip->GetUnit( i );
                                        if ( pBldUnit != NULL && pBldUnit->GetVehType( ) < CTransportData::num_types )
                                        {
                                            if ( awTypes[pBldUnit->GetVehType( )] )
                                            {
                            ...
                            if ( m_bNeedTrucks && iBldg == CStructureData::light_0 )
                            {
...
#ifdef _LOGOUT
                            logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC,
                                       "CAIGoalMgr::player %d vehicle factory goal %d task %d priority %d ", m_iPlayer,
                                       pTask->GetGoalID( ), pTask->GetID( ), pTask->GetPriority( ) );
#endif
                        }
                    }
                    ...
                    if ( bNeedDefenses )
                    {
                        ...
                        if ( iBldg >= CStructureData::fort_1 && iBldg <= CStructureData::fort_2 )
                        {
    ...
    while ( pos != NULL )
    {
        ...
        if ( pUnit != NULL )
        {
            ...
            if ( pUnit->GetType( ) == CUnit::building )
            {
                ...
                if ( pTask->GetTaskParam( ORDER_TYPE ) == PRODUCTION_ORDER )
                {
                    if ( pTask->GetTaskParam( PRODUCTION_TYPE ) == CStructureData::UTvehicle ||
...
void CAIGoalMgr::UpdateStagingTasks( void )
{
    ...
    while ( pos != NULL )
    {
        ...
        if ( pTask != NULL )
        {
            ...
            if ( pTask->GetID( ) == IDT_PREPAREWAR )
            {
                ...
                if ( pTask->GetGoalID( ) == IDG_SEAINVADE && pInvadeTask == NULL )
                {
                    pInvadeTask = pTask->CopyTask( );
                    pInvadeTask->SetGoalID( pTask->GetGoalID( ) );
                }
                else if ( pTask->GetGoalID( ) == IDG_ADVDEFENSE && pDefenseTask == NULL )
                {
                    pDefenseTask = pTask->CopyTask( );
                    pDefenseTask->SetGoalID( pTask->GetGoalID( ) );
                }
                else if ( pTask->GetGoalID( ) == IDG_LANDWAR && pWarTask == NULL )
                {
    ...
    if ( pInvadeTask != NULL )
    {
        for ( int i = 0; i < MAX_TASKPARAMS; ++i ) pInvadeTask->SetTaskParam( i, 0 );
    }
    if ( pDefenseTask != NULL )
    {
        for ( int i = 0; i < MAX_TASKPARAMS; ++i ) pDefenseTask->SetTaskParam( i, 0 );
    }
    if ( pWarTask != NULL )
    {
        for ( int i = 0; i < MAX_TASKPARAMS; ++i ) pWarTask->SetTaskParam( i, 0 );
    }
    ...
    while ( pos != NULL )
    {
        ...
        if ( pUnit != NULL )
        {
            ...
            if ( pUnit->GetGoal( ) == IDG_ADVDEFENSE || pUnit->GetGoal( ) == IDG_LANDWAR )
            {
                ...
                case CTransportData::med_tank:
                case CTransportData::heavy_tank:
                    if ( pUnit->GetGoal( ) == IDG_ADVDEFENSE && pDefenseTask != NULL )
                    {
                        wCnt = pDefenseTask->GetTaskParam( CAI_TF_TANKS ) + 1;
                        pDefenseTask->SetTaskParam( CAI_TF_TANKS, wCnt );
                    }
                    else if ( pUnit->GetGoal( ) == IDG_LANDWAR && pWarTask != NULL )
                    {
                        wCnt = pWarTask->GetTaskParam( CAI_TF_TANKS ) + 1;
                ...
                case CTransportData::infantry_carrier:
                    if ( pUnit->GetGoal( ) == IDG_ADVDEFENSE && pDefenseTask != NULL )
                    {
                        wCnt = pDefenseTask->GetTaskParam( CAI_TF_IFVS ) + 1;
                        pDefenseTask->SetTaskParam( CAI_TF_IFVS, wCnt );
                    }
                    else if ( pUnit->GetGoal( ) == IDG_LANDWAR && pWarTask != NULL )
                    {
                        wCnt = pWarTask->GetTaskParam( CAI_TF_IFVS ) + 1;
                ...
                case CTransportData::heavy_art:
                    if ( pUnit->GetGoal( ) == IDG_ADVDEFENSE && pDefenseTask != NULL )
                    {
                        wCnt = pDefenseTask->GetTaskParam( CAI_TF_ARTILLERY ) + 1;
                        pDefenseTask->SetTaskParam( CAI_TF_ARTILLERY, wCnt );
                    }
                    else if ( pUnit->GetGoal( ) == IDG_LANDWAR && pWarTask != NULL )
                    {
                        wCnt = pWarTask->GetTaskParam( CAI_TF_ARTILLERY ) + 1;
                ...
                case CTransportData::rangers:
                    if ( pUnit->GetGoal( ) == IDG_ADVDEFENSE && pDefenseTask != NULL )
                    {
                        wCnt = pDefenseTask->GetTaskParam( CAI_TF_INFANTRY ) + 1;
                        pDefenseTask->SetTaskParam( CAI_TF_INFANTRY, wCnt );
                    }
                    else if ( pUnit->GetGoal( ) == IDG_LANDWAR && pWarTask != NULL )
                    {
                        wCnt = pWarTask->GetTaskParam( CAI_TF_INFANTRY ) + 1;
            ...
            else if ( pUnit->GetGoal( ) == IDG_SEAINVADE )
            {
                ...
                case CTransportData::med_tank:
                case CTransportData::light_art:
                    wCnt = pInvadeTask->GetTaskParam( CAI_TF_ARMOR ) + 1;
                    pInvadeTask->SetTaskParam( CAI_TF_ARMOR, wCnt );
                    break;
                case CTransportData::landing_craft:
                    wCnt = pInvadeTask->GetTaskParam( CAI_TF_LANDING ) + 1;
                    pInvadeTask->SetTaskParam( CAI_TF_LANDING, wCnt );
                    break;
                case CTransportData::gun_boat:
                    wCnt = pInvadeTask->GetTaskParam( CAI_TF_SHIPS ) + 1;
                    pInvadeTask->SetTaskParam( CAI_TF_SHIPS, wCnt );
                    break;
                case CTransportData::rangers:
                    wCnt = pInvadeTask->GetTaskParam( CAI_TF_MARINES ) + 1;
    ...
    if ( pInvadeTask != NULL )
    {
        logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "UpdateStagingTasks() for goal %d task %d IDG_SEAINVADE assignments",
    ...
    if ( pDefenseTask != NULL )
    {
        logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "UpdateStagingTasks() for goal %d task %d IDG_ADVDEFENSE assignments",
    ...
    if ( pWarTask != NULL )
    {
        logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "UpdateStagingTasks() for goal %d task %d IDG_LANDWAR assignments",
    ...
    if ( pInvadeTask != NULL && pTask != NULL )
    {
        ...
        while ( i )
        {
            ...
            if ( pInvadeTask->GetTaskParam( wUnitCat ) &&
                 ( pInvadeTask->GetTaskParam( wUnitCat ) < pTask->GetTaskParam( wUnitCat ) ) )
            {
                ...
                if ( pDefenseTask != NULL )
                {
                    ...
                    pActualTask = m_plTasks->GetTask( pDefenseTask->GetID( ), pDefenseTask->GetGoalID( ) );
                    if ( pActualTask != NULL )
                    {
                        // task has less of this unit catagory
                        // assigned than is needed
                        if ( pDefenseTask->GetTaskParam( wUnitCat ) &&
                             ( pDefenseTask->GetTaskParam( wUnitCat ) < pActualTask->GetTaskParam( wUnitCat ) ) )
                            ConsiderReassignment( pTask, pInvadeTask, pActualTask, wUnitCat );
                    }
                }
                else if ( pWarTask != NULL )
                {
                    // get actual IDG_LANDWAR task
                    pActualTask = m_plTasks->GetTask( pWarTask->GetID( ), pWarTask->GetGoalID( ) );
                    if ( pActualTask != NULL )
                    {
                        // task has less of this unit catagory
                        // assigned than is needed
                        if ( pWarTask->GetTaskParam( wUnitCat ) &&
                             ( pWarTask->GetTaskParam( wUnitCat ) < pActualTask->GetTaskParam( wUnitCat ) ) )
                            ConsiderReassignment( pTask, pInvadeTask, pActualTask, wUnitCat );
                    }
                }
            }
    ...
    {
        for ( wUnitCat = CAI_TF_TANKS; wUnitCat <= CAI_TF_INFANTRY; ++wUnitCat )
        {
            if ( pDefenseTask->GetTaskParam( wUnitCat ) &&
                 ( pDefenseTask->GetTaskParam( wUnitCat ) < pTask->GetTaskParam( wUnitCat ) ) )
            {
                ...
                if ( pWarTask != NULL )
                {
                    ...
                    if ( pActualTask != NULL )
                    {
                        ...
                        if ( pWarTask->GetTaskParam( wUnitCat ) &&
                             ( pWarTask->GetTaskParam( wUnitCat ) < pActualTask->GetTaskParam( wUnitCat ) ) )
                            ConsiderReassignment( pTask, pDefenseTask, pActualTask, wUnitCat );
                    }
                }
            }
        }
    ...
    if ( pWarTask != NULL && pTask != NULL )
    {
        ...
        {
            if ( pWarTask->GetTaskParam( wUnitCat ) &&
                 ( pWarTask->GetTaskParam( wUnitCat ) < pTask->GetTaskParam( wUnitCat ) ) )
            {
                ...
                if ( pDefenseTask != NULL )
                {
                    ...
                    if ( pActualTask != NULL )
                    {
                        ...
                        if ( pDefenseTask->GetTaskParam( wUnitCat ) &&
                             ( pDefenseTask->GetTaskParam( wUnitCat ) < pActualTask->GetTaskParam( wUnitCat ) ) )
                            ConsiderReassignment( pTask, pWarTask, pActualTask, wUnitCat );
                    }
                }
            }
        }
...
//
void CAIGoalMgr::ConsiderReassignment( CAITask* pToTask, CAITask* pCntTask, CAITask* pFromTask, WORD wUnitCat )
{
    // how many units of that catagory can we transfer
    int iTransfering = pToTask->GetTaskParam( wUnitCat ) - pCntTask->GetTaskParam( wUnitCat );
    if ( iTransfering <= 0 )
        return;
    ...
    while ( pos != NULL )
    {
        ...
        if ( pUnit != NULL )
        {
            ...
            if ( pUnit->GetGoal( ) != pFromTask->GetGoalID( ) || pUnit->GetTask( ) != pFromTask->GetID( ) )
                continue;
            ...
            else if ( pToTask->GetGoalID( ) == IDG_ADVDEFENSE || pToTask->GetGoalID( ) == IDG_LANDWAR )
            {
            ...
            m_pMap->m_pMapUtil->FindStagingHex( hexVeh, 2, 2, pUnit->GetTypeUnit( ), hexVeh, FALSE );
...
#ifdef _LOGOUT
            logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC,
                       "CAIGoalMgr::ConsiderReassignment() for player %d unit %ld a %d vehicle ", pUnit->GetOwner( ),
                       pUnit->GetID( ), pUnit->GetTypeUnit( ) );
            logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "has been reassigned to goal %d task %d and going to %d,%d \n",
...
{
#ifdef _LOGOUT
    logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "CAIGoalMgr::UpdateProductionTasks() for %d executing ", m_iPlayer );
#endif
    ...
    WORD awTypes[CTransportData::num_types];
    int  i = 0;
    for ( ; i < CTransportData::num_types; ++i ) awTypes[i] = 0;
    SetAssaultStagingVehicle( awTypes );
    int iBestRatio = 0;
        ...
        if ( pTask != NULL )
        {
            ...
            if ( pTask->GetTaskParam( ORDER_TYPE ) == PRODUCTION_ORDER )
            {
                     ...
                     pTask->GetTaskParam( PRODUCTION_TYPE ) == CStructureData::UTshipyard )
                {
                    ...
                    if ( iVehDead < m_iNumUnits && iVeh == iVehDead )
                    {
                        if ( !pTask->GetPriority( ) )
                        {
                            // reset to standard priority
                    ...
                    if ( m_pwaUnits[iVeh] > m_pwaVehGoals[iVeh] )
                    {
                        pTask->SetPriority( (BYTE)0 );
                    }
                    else  // this is supposed to restart production
                    {
                        ...
                        if ( pStdTask != NULL )
                        {
                            ...
                            // only for trucks!
                            if ( iVeh == CTransportData::heavy_truck || iVeh == CTransportData::med_truck )
                            {
                                if ( m_pwaUnits[iVeh] < MINIMUM_IDLE_TRUCKS_AI || m_bNeedTrucks )
                                {
                                    pTask->SetPriority( 99 );
                                    m_bNeedTrucks = FALSE;
                                }
                            ...
                            if ( !m_pwaUnits[CTransportData::light_scout] && iVeh == CTransportData::light_scout )
                            {
                                pTask->SetPriority( 98 );
                            }
                            // ceiling
...
#ifdef _LOGOUT
                    logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC,
                               "CAIGoalMgr::player %d set production goal %d task %d new priority %d ", m_iPlayer,
                               pTask->GetGoalID( ), pTask->GetID( ), pTask->GetPriority( ) );
                    logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC,
                               "for vehicle type %d, with count=%d and goals=%d staging=%d ", iVeh, m_pwaUnits[iVeh],
                               m_pwaVehGoals[iVeh], awTypes[iVeh] );
#endif
                }
            }
        }
...
void CAIGoalMgr::UpdateSummarys( CAIMsg* pMsg )
{
    ...
    */
...
#ifdef _LOGOUT
            logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "CAIGoalMgr::UpdateSummarys() player %d for BUILDING %d, count=%d ",
                       m_iPlayer, iAt, wCnt );
#endif
        }
    }
    // a new one of our vehicles was just produced
    else if ( pMsg->m_iMsg == CNetCmd::veh_new && pMsg->m_idata3 == m_iPlayer )
    {
...
#ifdef _LOGOUT
            logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "CAIGoalMgr::UpdateSummarys() player %d for VEHICLE %d, count=%d ",
                       m_iPlayer, iAt, wCnt );
#endif
        }
    }
    ...
    // as the offset
    //
    else if ( pMsg->m_iMsg == CNetCmd::delete_unit && pMsg->m_idata3 == m_iPlayer )
    {
        ...
        if ( pUnit->GetType( ) == CUnit::building )
        {
            ...
            if ( iAt < m_iNumBldgs && m_pwaBldgs[iAt] )
            {
                WORD wCnt       = m_pwaBldgs[iAt] - 1;
                m_pwaBldgs[iAt] = wCnt;
#ifdef _LOGOUT
                logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC,
                           "CAIGoalMgr::UpdateSummarys() player %d reduced for BUILDING %d, count=%d ", m_iPlayer, iAt,
                           wCnt );
#endif
            }
        }
        else if ( pUnit->GetType( ) == CUnit::vehicle )
        {
            ...
            int iAt = pUnit->GetTypeUnit( );
            if ( iAt < m_iNumUnits && m_pwaUnits[iAt] )
            {
                WORD wCnt       = m_pwaUnits[iAt] - 1;
                m_pwaUnits[iAt] = wCnt;
#ifdef _LOGOUT
                logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC,
                           "CAIGoalMgr::UpdateSummarys() player %d for VEHICLE %d, count=%d ", m_iPlayer, iAt, wCnt );
#endif
            }
        }
    }
    ...
    // set up opfor for
    //
    else if ( ( pMsg->m_iMsg == CNetCmd::veh_new || pMsg->m_iMsg == CNetCmd::bldg_new ||
                pMsg->m_iMsg == CNetCmd::delete_unit ) &&
              pMsg->m_idata3 != m_iPlayer )
    {
        // the opfor involved
...
void CAIGoalMgr::InitTasks( CAIGoal* pGoal )
{
    ...
    if ( pGoal != NULL )
    {
        ...
        while ( wTaskID )
        {
            ...
            CAITask* pTask = plTaskList->GetTask( wTaskID, FALSE );
       ...
       //  BUGBUG this function must yield
                ...
                {
                    WORD wBldg = CStructureData::apartment_1_1 + pGameData->GetRandom( 3 );
                ...
                {
                    WORD wBldg = CStructureData::office_2_1 + pGameData->GetRandom( 3 );
                ...
                if ( pNewTask->GetID( ) == IDT_BUILDRD || pNewTask->GetID( ) == IDT_BUILDREFINERY )
                {
                    wQty = pNewTask->GetTaskParam( PRODUCTION_QTY ) + pGameData->m_iSmart;
                    pNewTask->SetTaskParam( PRODUCTION_QTY, wQty );
                }

                if ( pNewTask->GetTaskParam( ORDER_TYPE ) == PRODUCTION_ORDER )
                {
                    if ( pNewTask->GetTaskParam( PRODUCTION_TYPE ) == CStructureData::UTvehicle ||
                         pNewTask->GetTaskParam( PRODUCTION_TYPE ) == CStructureData::UTshipyard )
                    {
                        // get id of the vehicle being produced
                ...
                {
                    if ( pNewTask->GetTaskParam( ORDER_TYPE ) == PRODUCTION_ORDER &&
                         pNewTask->GetTaskParam( PRODUCTION_TYPE ) == CStructureData::UTvehicle )
                    {
                        wQty = pNewTask->GetTaskParam( PRODUCTION_QTY ) + (WORD)pGameData->m_iSmart;
                        pNewTask->SetTaskParam( PRODUCTION_QTY, wQty );
                    }
                }
...
void CAIGoalMgr::InitBldgSummary( void )
{
    ...
    while ( pos != NULL )
    {
        ...
        if ( pUnit != NULL )
        {
            ...
            if ( pCopyCBuilding->m_aiDataOut[CAI_TYPEBUILD] < m_iNumBldgs )
            {
        ...
        if ( pTask != NULL )
        {
            ...
            if ( pTask->GetTaskParam( ORDER_TYPE ) == PRODUCTION_ORDER )
            {
                     ...
                     pTask->GetTaskParam( PRODUCTION_TYPE ) == CStructureData::UTshipyard )
                {
                    ...
                    if ( pTask->GetTaskParam( PRODUCTION_ID1 ) != pUnit->GetTypeUnit( ) &&
                         pTask->GetTaskParam( PRODUCTION_ID2 ) != pUnit->GetTypeUnit( ) )
                        continue;
...
void CAIGoalMgr::LaunchAssault( CAITask* pTask )
{
    ...
    {
#ifdef _LOGOUT
        logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "CAIGoalMgr::LaunchAssault() player %d goal %d task %d no opfors ",
                   m_iPlayer, pTask->GetGoalID( ), pTask->GetID( ) );
#endif
        return;  // could not launch, just return?
    }

    CHexCoord hexCity( pTask->GetTaskParam( CAI_LOC_X ), pTask->GetTaskParam( CAI_LOC_Y ) );
...
#ifdef _LOGOUT
                logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC,
                           "CAIGoalMgr::LaunchAssault() player %d goal %d task %d no opfor target ", m_iPlayer,
                           pTask->GetGoalID( ), pTask->GetID( ) );
#endif
                return;
            }
...
{
#ifdef _LOGOUT
    logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "CAIGoalMgr::LaunchAssault() player %d goal %d task %d at %d,%d ",
    ...
    CTransportData const* pVehData = pGameData->GetTransportData( iTypeVeh );
    if ( pVehData == NULL )
        return;
    ...
    CHexCoord hex;
    int       iDeltaX = hex.Wrap( pTask->GetTaskParam( CAI_PREV_X ) - pTask->GetTaskParam( CAI_LOC_X ) );
    int       iDeltaY = hex.Wrap( pTask->GetTaskParam( CAI_PREV_Y ) - pTask->GetTaskParam( CAI_LOC_Y ) );
...
void CAIGoalMgr::ReconInForce( CAITask* pTask, CHexCoord hcTo )
{
         ...
         pTask->GetGoalID( ) == IDG_SEAINVADE )
    {
        ...
        if ( pGameHex != NULL )
        {
            ...
            CTransportData const* pVehData = pGameData->GetTransportData( CTransportData::cruiser );
            if ( pVehData == NULL )
                return;  // what else can I do at this point???

            // get some patrol points from the area, that this ship
            // can travel
            CAIHex* pHexes = m_pMap->m_pMapUtil->FindWaterPatrol( hcMid, pVehData );
            if ( pHexes == NULL )
                return;  // what else can I do at this point???
    ...
    else
    {
        ...
        if ( pGameHex != NULL )
        {
            ...
            if ( pGameHex->GetType( ) == CHex::ocean || pGameHex->GetType( ) == CHex::river )
            {
                hex = hcMid;
...
#ifdef _LOGOUT
    logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "CAIGoalMgr::CancelAssault() player %d goal %d task %d ", m_iPlayer,
               pTask->GetGoalID( ), pTask->GetID( ) );
#endif
    ...
    while ( pos != NULL )
    {
        ...
        if ( pUnit != NULL )
        {
            ...
            if ( pUnit->GetTask( ) == pTask->GetID( ) && pUnit->GetGoal( ) == pTask->GetGoalID( ) )
            {
                ...
                m_pMap->m_pMapUtil->FindStagingHex( hexVeh, 1, 1, pUnit->GetTypeUnit( ), hexVeh );
...
// selected to be the destination of another unit on the task
//
void CAIGoalMgr::UpdateTaskForce( CAITask* pTask, CHexCoord& hcStart, CHexCoord& hcEnd, WORD wNewTask )
{
    ...
    while ( pos != NULL )
    {
        ...
        if ( pUnit != NULL )
        {
            ...
            if ( pUnit->GetTask( ) == pTask->GetID( ) && pUnit->GetGoal( ) == pTask->GetGoalID( ) )
            {
                ...
                if ( !bIsOnBoard )
                {
                    m_pMap->m_pMapUtil->FindStagingHex( hcStart.X( ), hcStart.Y( ), hcEnd.X( ), hcEnd.Y( ), hex,
                                                        pUnit->GetTypeUnit( ), TRUE );
...
//
void CAIGoalMgr::FindAssaultTarget( CHexCoord& hexTarget, CAITask* pTask, CAIOpFor* pOpFor )
{
    int iTarget = 0;
    if ( pGameData->m_iSmart > 1 )
    {
    ...
    pGameData->FindBuilding( iTarget, pOpFor->GetPlayerID( ), hexTarget );

    // no target found
    if ( hexTarget.X( ) == pTask->GetTaskParam( CAI_LOC_X ) && hexTarget.Y( ) == pTask->GetTaskParam( CAI_LOC_Y ) )
    {
        if ( pTask->GetGoalID( ) == IDG_PIRATE || pTask->GetGoalID( ) == IDG_SEAWAR ||
             pTask->GetGoalID( ) == IDG_SEAINVADE )
...
#ifdef _LOGOUT
    logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC,
               "CAIGoalMgr::FindAssaultTarget() player %d goal %d task %d  near %d,%d  terrain %d", m_iPlayer,
               pTask->GetGoalID( ), pTask->GetID( ), hexTarget.X( ), hexTarget.Y( ), iWhere );
#endif
    ...
    for ( int i = 0; i < 4; ++i )
    {
             ...
             pTask->GetGoalID( ) == IDG_SEAINVADE )
        {
            if ( pTask->GetGoalID( ) == IDG_SEAINVADE )
                pVehData = pGameData->GetTransportData( CTransportData::landing_craft );
            else
                pVehData = pGameData->GetTransportData( CTransportData::cruiser );
            if ( pVehData == NULL )
        ...
        {
            pVehData = pGameData->GetTransportData( CTransportData::med_scout );
            if ( pVehData == NULL )
                return FALSE;  // what else can I do at this point???
...
//
void CAIGoalMgr::ConvertPatrolPoints( CAIHex* pHexes, CHexCoord& hcStart, CHexCoord& hcEnd )
{
    int       iDeltaX = 0;
    int       iDeltaY = 0;
    int       iDelta;
...
void CAIGoalMgr::GetNewStagingArea( CAITask* pTask )
{
    ...
    else  // this is a restaging, get old size
    {
        ...
        m_pMap->m_pMapUtil->FlagStagingArea( FALSE, pTask->GetTaskParam( CAI_LOC_X ), pTask->GetTaskParam( CAI_LOC_Y ),
    ...
    if ( pTask->GetGoalID( ) == IDG_PIRATE || pTask->GetGoalID( ) == IDG_SEAINVADE )
    ...
    */
    ...
    while ( pos != NULL )
    {
        ...
        if ( pUnit != NULL )
        {
            ...
            logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC,
                       ...
                       pUnit->GetOwner( ), pUnit->GetID( ), hexBldg.X( ), hexBldg.Y( ), pTask->GetGoalID( ),
                       pTask->GetID( ) );
#endif

            bIsStaged = m_pMap->m_pMapUtil->GetStagingAreaNear( hexBldg, hcFrom, hcTo, iWidth, iHeight, iType );
        }
        if ( bIsStaged )
            break;
    }
    ...
    if ( !bIsStaged && wCnt )
    {
        ...
        if ( pTask->GetGoalID( ) == IDG_PIRATE )
            m_pMap->m_pMapUtil->GetWaterStagingArea( CTransportData::cruiser, hcFrom, hcTo, iWidth, iHeight );
        else if ( pTask->GetGoalID( ) == IDG_SEAINVADE )
            m_pMap->m_pMapUtil->GetWaterStagingArea( CTransportData::landing_craft, hcFrom, hcTo, iWidth, iHeight );
    ...
    if ( !bIsStaged && !wCnt )
    {
        ...
        if ( m_pMap->m_pMapUtil->m_RocketHex.X( ) > pGameData->m_iHexPerBlk )
            hexStart.X( ( m_pMap->m_pMapUtil->m_RocketHex.X( ) / pGameData->m_iHexPerBlk ) * pGameData->m_iHexPerBlk );
        else
            hexStart.X( 0 );

        if ( m_pMap->m_pMapUtil->m_RocketHex.Y( ) > pGameData->m_iHexPerBlk )
            hexStart.Y( ( m_pMap->m_pMapUtil->m_RocketHex.Y( ) / pGameData->m_iHexPerBlk ) * pGameData->m_iHexPerBlk );
        else
            hexStart.Y( 0 );
...
#ifdef _LOGOUT
            logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC,
                       "CAIGoalMgr::GetNewStagingArea() player %d no unit found nearby for goal %d task %d ", m_iPlayer,
                       pTask->GetGoalID( ), pTask->GetID( ) );
            logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "CAIGoalMgr::GetNewStagingArea() using %d,%d for a reference \n",
                       hexBldg.X( ), hexBldg.Y( ) );
#endif
...
void CAIGoalMgr::GetStagingArea( CAITask* pTask )
{
    ...
    int i = 0;
    for ( ; i < MAX_TASKPARAMS; ++i ) pTask->SetTaskParam( i, 0 );
    ...
    if ( !hcFrom.X( ) && !hcFrom.Y( ) && !hcTo.X( ) && !hcTo.Y( ) )
        return;
    ...
    if ( pTask->GetGoalID( ) == IDG_PIRATE )
    {
        ...
        if ( !pTask->GetTaskParam( CAI_TF_CRUISERS ) && !pTask->GetTaskParam( CAI_TF_DESTROYERS ) )
        {
            if ( CanBuildVehType( CTransportData::cruiser ) )
    ...
    if ( pTask->GetGoalID( ) == IDG_SEAINVADE )
    {
        ...
        if ( iTotalUnits > 0 )
        {
            ...
            if ( pTask->GetTaskParam( CAI_TF_ARMOR ) > pTask->GetTaskParam( CAI_TF_MARINES ) )
                pTask->SetTaskParam( CAI_TF_ARMOR, pTask->GetTaskParam( CAI_TF_MARINES ) );
    ...
    if ( pTask->GetGoalID( ) == IDG_ADVDEFENSE || pTask->GetGoalID( ) == IDG_LANDWAR )
    {
        ...
        if ( !i )
        {
            ...
            while ( pos != NULL )
            {
                ...
                if ( pUnit != NULL )
                {
                    ...
                    if ( !pUnit->GetTask( ) && ( pUnit->GetTypeUnit( ) >= CTransportData::light_tank &&
        ...
        if ( !pGameData->m_iSmart )
        {
            if ( !pTask->GetTaskParam( CAI_TF_INFANTRY ) )
            {
                pTask->SetTaskParam( CAI_TF_INFANTRY, m_pwaVehGoals[CTransportData::infantry] / 2 );
            }
        }
...
DWORD CAIGoalMgr::GetOpforTarget( int iOpForID, int iVehType )
{
    ...
    while ( pos != NULL )
    {
        ...
        if ( pOpforUnit != NULL )
        {
            ...
            pVehData = pGameData->GetTransportData( pOpforUnit->GetTypeUnit( ) );
            if ( pVehData == NULL )
                continue;
            ...
            if ( !pOpforUnit->GetParam( CAI_FIRERATE ) )
            {
                ...
                if ( pOpforUnit->GetType( ) == CUnit::building )
                {
                    iRating = 0;
                    EnterCriticalSection( &cs );
                    CBuilding* pBuilding = theBuildingMap.GetBldg( pOpforUnit->GetID( ) );
                    if ( pBuilding != NULL )
                    {
                ...
                else if ( pOpforUnit->GetType( ) == CUnit::vehicle )
                {
                    iRating = 0;
                    EnterCriticalSection( &cs );
                    CVehicle* pVehicle = theVehicleMap.GetVehicle( pOpforUnit->GetID( ) );
                    if ( pVehicle != NULL )
                        iRating = m_pMap->m_pMapUtil->AssessTarget( pVehicle, 0 );
                    LeaveCriticalSection( &cs );
                    // use target type to start offset to factor
                    i = NUM_COMBINED_UNITS * ( CStructureData::num_types + pOpforUnit->GetType( ) );
                }
                else
                    continue;
...
TryTryAgain:
    ...
    for ( iY = 0; iY < iDeltaY; ++iY )
    {
        ...
        for ( iX = 0; iX < iDeltaX; iX++ )
        {
            ...
            if ( !( bUnits & CHex::bldg ) && ( bUnits & ( CHex::ul | CHex::ur | CHex::ll | CHex::lr ) ) )
            {
                ...
                if ( !iKindOf || ( iKindOf == CAI_SOFTATTACK ) || ( iKindOf == CAI_NAVALATTACK ) )
                {
                    ...
                    for ( int iy = 0; iy < 2; ++iy )
                    {
                        ...
                        for ( int ix = 0; ix < 2; ++ix )
                        {
                            ...
                            if ( pVehicle != NULL )
                            {
                                iVehPlayer  = pVehicle->GetOwner( )->GetPlyrNum( );
                                iOpForID    = pVehicle->GetOwner( )->GetPlyrNum( );
                    ...
                    if ( pVehicle != NULL )
                    {
                        ...
                        {
                            EnterCriticalSection( &cs );
                            pVehicle = theVehicleMap.GetVehicle( dwOpForUnit );
                            if ( pVehicle != NULL )
                            {
                                iThreat = AssessThreat( pVehicle, iKindOf );
                            }
                            LeaveCriticalSection( &cs );
                        }
                        else if ( iHow == BEST_TARGET )
                        {
                            ...
                            if ( pVehicle != NULL )
                            {
                                iBest = m_pMap->m_pMapUtil->AssessTarget( pVehicle, iKindOf );
            ...
            if ( bUnits & CHex::bldg )
            {
                ...
                if ( !iOpForID && ( !iKindOf || ( iKindOf == CAI_HARDATTACK ) ) )
                {
                    EnterCriticalSection( &cs );
                    CBuilding* pBuilding = theBuildingHex.GetBuilding( hcAt );
                    if ( pBuilding != NULL )
                    {
                        iOpForID     = pBuilding->GetOwner( )->GetPlyrNum( );
                        iType        = pBuilding->GetData( )->GetType( );
                        dwOpForUnit  = pBuilding->GetID( );
                        bIsAbandoned = pBuilding->IsFlag( CUnit::abandoned );
                    }
                    LeaveCriticalSection( &cs );

                    // don't attack our own units
                    if ( iOpForID == m_iPlayer || bIsAbandoned )
                    {
                        dwOpForUnit = 0;
                        continue;
                    }
                    ...
                    {
                        EnterCriticalSection( &cs );
                        pBuilding = theBuildingMap.GetBldg( dwOpForUnit );
                        if ( pBuilding != NULL )
                        {
                            iThreat      = AssessThreat( pBuilding, iKindOf );
                            bIsAbandoned = pBuilding->IsFlag( CUnit::abandoned );
                    ...
                    else if ( iHow == BEST_TARGET )
                    {
                        ...
                        if ( pBuilding != NULL )
                        {
                            iBest = m_pMap->m_pMapUtil->AssessTarget( pBuilding, iKindOf );
    ...
    {
        logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC,
                   "CAIGoalMgr::GetOpForUnit() player %d unit %ld selected target %ld on first try", pUnit->GetOwner( ),
                   pUnit->GetID( ), dwSeekTo );
        logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC,
                   "CAIGoalMgr::GetOpForUnit() iHow=%d  [BEST=310 NEAREST=311 THREAT=312 WEAKEST=313]", iHow );
    }
#endif
    ...
    if ( !dwSeekTo )
    {
        if ( iArea < iSpotting )
            iArea = iSpotting;
        else if ( iArea < pGameData->m_iHexPerBlk && iHow == NEAREST_TARGET )
        {
        ...
        else  // unable to spot a target
        {
            ...
            //
            if ( !dwSeekTo && iHow == NEAREST_TARGET )
            {
                if ( pUnit->GetType( ) == CUnit::vehicle )
                {
                    pGameData->GetVehicleHex( pUnit->GetID( ), hexUnit );
                ...
                while ( pos != NULL )
                {
                    ...
                    if ( pTargetUnit != NULL )
                    {
                        ...
                        if ( iRange && ( iRange < iClosest ) )
                        {
                            ...
                            if ( pUnit->GetType( ) == CUnit::vehicle &&
                                 !m_pMap->m_pMapUtil->GetPathRating( hexUnit, hcAt, pUnit->GetTypeUnit( ) ) )
                                continue;
...
#ifdef _LOGOUT
    logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "\nCAIGoalMgr::KillOpfor() player %d to kill player %d", m_iPlayer,
               iOpFor );
#endif
        ...
        if ( pUnit != NULL )
        {
            ...
            if ( pUnit->GetType( ) == CUnit::vehicle )
            {
                ...
                if ( pUnit->GetTask( ) != IDT_PATROL && pUnit->GetTask( ) != IDT_SCOUT &&
                     pUnit->GetTask( ) != IDT_SEEKINWAR && pUnit->GetTask( ) != IDT_SEEKINRANGE &&
                ...
                // can unit get there?
                if ( !m_pMap->m_pMapUtil->GetPathRating( hexVeh, hexTarget, pUnit->GetTypeUnit( ) ) )
                    continue;
                ...
                m_pMap->m_pMapUtil->FindStagingHex( hexTarget, iSpotting, iSpotting, pUnit->GetTypeUnit( ), hexVeh,
                                                    FALSE );
...
void CAIGoalMgr::SetMapOpFor( void )
{
    ...
    if ( pOpFor != NULL )
    {
        ...
        pGameData->FindBuilding( CStructureData::rocket, pOpFor->GetPlayerID( ), hexRocket );
...
int CAIGoalMgr::GetOpForId( int iType, CHexCoord hex, BOOL bKnown )
{
    ...
    if ( pOpFor != NULL && pOpFor->GetPlayerID( ) != m_iPlayer )
...
//
void CAIGoalMgr::GetSeaPatrol( CAIUnit* pShip, CAITask* pTask, CTransportData const* ptdShip )
{
    CAIHex* phexPts  = NULL;
    int     iPattern = 0;
    ...
    if ( !pTask->GetTaskParam( 0 ) && !pTask->GetTaskParam( 1 ) )
    {
        phexPts = m_pMap->m_pMapUtil->FindWaterPatrol( pTask->GetGoalID( ) );
        if ( phexPts != NULL )
        {
            for ( int i = 0; i < NUM_PATROL_POINTS; i++ )
            {
    ...
    for ( int i = 0; i < NUM_PATROL_POINTS; ++i ) cAssigned[i] = 0;
    ...
    if ( !iPattern )
    {
        int i = pGameData->GetRandom( NUM_PATROL_POINTS );
    ...
    phexPts = m_pMap->m_pMapUtil->FindWaterPatrol( hexProtect, ptdShip );
    if ( phexPts != NULL )
    {
        for ( int i = 0; i < NUM_PATROL_POINTS; i++ )
        {
            ...
            while ( posU != NULL )
            {
                ...
                if ( pVeh != NULL )
                {
                    ...
                    if ( pVeh->GetType( ) != CUnit::vehicle )
                        continue;
                    if ( pVeh->GetGoal( ) != pTask->GetGoalID( ) || pVeh->GetTask( ) != pTask->GetID( ) )
                        continue;
    ...
    if ( pProtected != NULL )
    {
        ...
        while ( posC != NULL )
        {
            ...
            if ( pCrane != NULL )
            {
                ...
                if ( bSeaBuilding )
                {
                    if ( pCrane->GetTypeUnit( ) != CTransportData::landing_craft &&
                         pCrane->GetTypeUnit( ) != CTransportData::light_cargo )
                        continue;
                }
                else
                {
                    // determine if this is a crane
                    if ( pCrane->GetTypeUnit( ) != CTransportData::construction )
                        continue;
                }
       ...
       //  BUGBUG this function must yield
                ...
                if ( !iCnt )
                {
                    ...
                    else
                    {
                        ...
                        {
                            if ( !pCrane->GetGoal( ) && !pCrane->GetTask( ) )
                                pProtected = pCrane;
                        }
                    }
                }
...
pUnit->GetOwner(), pUnit->GetID(), pUnit->GetParamDW(CAI_PATROL) );
    ...
    CHexCoord hexProtect( 0, 0 );
    EnterCriticalSection( &cs );
    CBuilding* pBldg = theBuildingMap.GetBldg( pUnit->GetParamDW( CAI_PATROL ) );
    if ( pBldg != NULL )
        hexProtect = pBldg->GetExitHex( );
    else
    {
        CVehicle* pVehicle = theVehicleMap.GetVehicle( pUnit->GetParamDW( CAI_PATROL ) );
        if ( pVehicle != NULL )
    ...
    {
#ifdef _LOGOUT
        logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "CAIGoalMgr::GetPatrolHex() patrol %ld objective not found ",
...
#ifdef _LOGOUT
    logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "CAIGoalMgr::GetPatrolHex() for player %d unit %ld to patrol %ld at %d,%d",
               pUnit->GetOwner( ), pUnit->GetID( ), pUnit->GetParamDW( CAI_PATROL ), hexProtect.X( ), hexProtect.Y( ) );
#endif

    // right now pick a staging hex within spotting range of building return in hex
    m_pMap->m_pMapUtil->FindStagingHex( hexProtect, iSpotting, iSpotting, pUnit->GetTypeUnit( ), hex, FALSE );
    ...
    else
    {
        ...
        // hex = hexVeh;
        if ( !m_pMap->m_pMapUtil->GetPathRating( hexVeh, hex, pVehData->GetType( ) ) )
            hex = hexVeh;
    }
}
...
CAITask* CAIGoalMgr::GetPatrolTask( int iType )
{
    ...
    if ( pPickedTask->GetGoalID( ) == IDG_SEAWAR || pPickedTask->GetGoalID( ) == IDG_REPELL ||
         pPickedTask->GetGoalID( ) == IDG_PIRATE || pPickedTask->GetGoalID( ) == IDG_SEAINVADE ||
