...
class CDataTransfer
{
  ...
  public:
    ...
    BOOL         SendMore( ) { return FillWindow( ); }
    ...
    DWORD UntransferredDataAmount( ) const { return m_totalSize - m_transferred; }

    DWORD TransferredDataAmount( ) const { return m_acked; }

    BOOL Done( ) const { return m_error || ( m_totalSize == m_acked ); }

    DWORD GetError( ) const { return m_error; }


    void SetError( DWORD err ) { m_error = err; }

    BOOL Sending( ) const { return m_sending; }

  protected:
    DWORD FreeWindowSpace( ) const { return m_windowSize - ( m_transferred - m_acked ); }
    ...
    BOOL WaitForEmptyWindow( ) { return WaitForSpace( m_windowSize ); }
