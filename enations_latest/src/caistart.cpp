...
void CAIStart::PickStartHex( void )
{
    ...
    logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "CAIStart::PickStartHex " );
    logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "from block at %d,%d ", m_iBlockX, m_iBlockY );
    logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "from %d,%d for %d hexes ", m_iBaseX, m_iBaseY, m_iBlockSize );
#endif
...
#ifdef _LOGOUT
        dwEnd = timeGetTime( );
        logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "start map took %ld ticks for map size of %d ", ( dwEnd - dwStart ),
                   m_iMapSize );
        logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "picking %d,%d \n", m_iBlockX, m_iBlockY );
#endif
    ...
    logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "start map took %ld ticks for map size of %d ", ( dwEnd - dwStart ),
               m_iMapSize );
    logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "picking %d,%d ", m_iBlockX, m_iBlockY );
#endif
}
...
BOOL CAIStart::IsHexValidToBuild( CAIHex* paiHex )
{
    ...
    if ( iTerrain == CHex::ocean || iTerrain == CHex::river || iTerrain == CHex::swamp || iTerrain == CHex::desert ||
         iTerrain == CHex::mountain || iTerrain == CHex::hill )
        return FALSE;
