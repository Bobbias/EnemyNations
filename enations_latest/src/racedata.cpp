...
CRaceDef* CRaceDef::Init( CRaceDef* pRd )
{
    ...
    for ( int iTyp = 0; iTyp < NUM_START_TYPES; iTyp++ )
        for ( int iOn = 0; iOn < num_supplies; iOn++ ) dwPos[iTyp][iOn] = pMmio->ReadShort( );
    pMmio->AscendChunk( );
    ...
    pMmio->DescendList( 'R', 'A', 'C', 'E' );
    for ( int iOn = 0; iOn < CRaceDef::m_iNumRaces; iOn++ ) pRd[iOn].InitText( pMmio );
    delete pMmio;
...
CRaceDef::CRaceDef( )
{
    ...
    for ( int iOn = 0; iOn < num_race; iOn++ ) m_fRace[iOn] = 1.0;
...
void CRaceDef::AssertValid( ) const
{
    ...
    for ( int iOn = 0; iOn < num_race; iOn++ ) ASSERT( ( 0.5 <= m_fRace[iOn] ) && ( m_fRace[iOn] <= 2.0 ) );

    for ( int iTyp = 0; iTyp < NUM_START_TYPES; iTyp++ )
        for ( int iOn = 0; iOn < num_supplies; iOn++ ) ASSERT( m_iPos[iTyp][iOn] <= 32000 );
}
...
CInitData::CInitData( )
{
    ...
    for ( int iOn = 0; iOn < CRaceDef::num_race; iOn++ ) m_fRace[iOn] = 1.0;
...
void CInitData::Serialize( CArchive& ar )
{
    ...
    if ( ar.IsStoring( ) )
    {
        for ( int iOn = 0; iOn < CRaceDef::num_race; iOn++ ) ar << m_fRace[iOn];

        for ( int iOn = 0; iOn < CRaceDef::num_supplies; iOn++ ) ar << m_iPos[iOn];
    }
    else
    {
        for ( int iOn = 0; iOn < CRaceDef::num_race; iOn++ ) ar >> m_fRace[iOn];
...
void CInitData::Init( )
{
    ...
    pMmio->DescendRiff( 'C', 'R', 'A', 'T' );
    pMmio->DescendList( 'R', 'A', 'C', 'E' );
    for ( int iOn = 0; iOn < NUM_RACE_CHAR; iOn++ ) m_raceAttrib[iOn].InitData( *pMmio );
    for ( iOn = 0; iOn < NUM_INIT_SPLYS; iOn++ ) m_posAttrib[iOn].InitData( *pMmio );
    ...
    pMmio->DescendRiff( 'L', 'A', 'N', 'G' );
    pMmio->DescendList( 'R', 'A', 'C', 'E' );
    for ( iOn = 0; iOn < NUM_RACE_CHAR; iOn++ ) m_raceAttrib[iOn].InitText( *pMmio );
    for ( iOn = 0; iOn < NUM_INIT_SPLYS; iOn++ ) m_posAttrib[iOn].InitText( *pMmio );
    delete pMmio;
...
void CInitData::AssertValid( ) const
{
    ...
    for ( int iOn = 0; iOn < NUM_RACE_CHAR; iOn++ ) ASSERT_VALID( &m_raceAttrib[iOn] );
    for ( iOn = 0; iOn < NUM_INIT_SPLYS; iOn++ ) ASSERT_VALID( &m_posAttrib[iOn] );
}
#endif
...
void CRaceDefinition::Serialize( CArchive& ar )
{
    ...
    if ( ar.IsStoring( ) )
    {
        ...
        for ( int iOn = 0; iOn < NUM_RACE_CHAR; iOn++ ) ar << m_iRace[iOn];
        for ( int iTyp = 0; iTyp < NUM_START_TYPES; iTyp++ )
            for ( iOn = 0; iOn < NUM_INIT_SPLYS; iOn++ ) ar << m_iPos[iTyp][iOn];
