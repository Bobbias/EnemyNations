...
//
void CAIMapUtil::FindCentralHex( CAIHex* pBaseHex, int iWidthX, int iWidthY, CHexCoord& hexFound )
{
#ifdef _LOGOUT
    logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "\nFindCentralHex() player %d for w=%d h=%d ", m_iPlayer, iWidthX,
               iWidthY );
    logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "closest to %d,%d \n", pBaseHex->m_iX, pBaseHex->m_iY );
#endif
    ...
    for ( int iy = 0; iy < iDeltaY; iy++ )
    {
        ...
        for ( int ix = 0; ix < iDeltaX; ix++ )
        {
            ...
            if ( theMap.FoundationCost( hex, CStructureData::rocket, 0, (CVehicle const*)-1 ) < 0 )
                continue;
        ...
        for ( iY = 0; iY < iDeltaY; iY++ )
        {
            ...
            for ( iX = 0; iX < iDeltaX; iX++ )
            {
                ...
                for ( int j = 0; j < NUM_PATROL_POINTS; ++j )
                {
                    ...
                    iDist = pGameData->GetRangeDistance( hcRay, hcTest );
...
// by the ptdShip
//
CAIHex* CAIMapUtil::FindWaterPatrol( CHexCoord& hexAt, CTransportData const* ptdShip )
{
...
void CAIMapUtil::FindSpecialHex( int iBldg, int iWidthX, int iWidthY, CHexCoord& hexFound )
{
#ifdef _LOGOUT
    ...
    //
    if ( iBldg == CStructureData::fort_1 || iBldg == CStructureData::fort_2 || iBldg == CStructureData::fort_3 )
    {
            ...
            if ( pUnit != NULL )
            {
                ...
                case CStructureData::copper:
                    ...
                    if ( iBestForts == pUnit->GetParam( CAI_ACCURACY ) )
                    {
                        ...
                        CBuilding* pBuilding = theBuildingMap.GetBldg( pUnit->GetID( ) );
                        if ( pBuilding != NULL )
        ...
        {
            EnterCriticalSection( &cs );
            CBuilding* pBuilding = theBuildingMap.GetBldg( pBestBldg->GetID( ) );
            if ( pBuilding != NULL )
                hexBase = pBuilding->GetExitHex( );
            LeaveCriticalSection( &cs );

            // increase number of forts assigned to this building
            pBestBldg->SetParam( CAI_ACCURACY, pBestBldg->GetParam( CAI_ACCURACY ) + 1 );

#ifdef _LOGOUT
            logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "FindSpecialHex() based from a %d building at %d,%d ",
        ...
        while ( iStep < iMaxSteps )
        {
            ...
            for ( int iY = 0; iY < iDeltay; ++iY )
            {
                ...
                for ( int iX = 0; iX < iDeltax; ++iX )
                {
                    ...
                    // use the game's opinion of building this building
                    if ( theMap.FoundationCost( hcAt, iBldg, 0, (CVehicle const*)-1 ) < 0 )
                        continue;

                    // consider if this is buildable
                    if ( !AreHexesOpen( MAX_ADJACENT, hcAt, iWidthX, iWidthY ) )
                        continue;
                    ...
                    iRating = pGameData->GetRangeDistance( hexRocket, hcAt );
...
#ifdef _LOGOUT
        logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "FindSpecialHex() not found \n" );
#endif
    }
...
int CAIMapUtil::AssessTarget( CVehicle* pVeh, int iKindOf )
{
    ...
    case CTransportData::construction:
        if ( pGameData->m_iSmart )
        {
            ...
            iAssessment += pGameData->GetRandom( ( pGameData->m_iSmart * 2 ) );
        }
        else
            iAssessment += 1;
    default:
    ...
    for ( iY = 0; iY < iDeltaY; ++iY )
    {
        ...
        for ( iX = 0; iX < iDeltaX; ++iX )
        {
            ...
            if ( theMinerals.Lookup( hex, pmn ) )
            {
                ...
                {
#ifdef _LOGOUT
                    logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "player %d found %d at %d,%d ", m_iPlayer, iMaterial,
                               hex.X( ), hex.Y( ) );
#endif
                    ...
                    {
#ifdef _LOGOUT
                        logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "Insufficient density of %d with iDenseFactor=%d ",
                    ...
                    if ( AreHexesOpen( MAX_ADJACENT, hex, iWidthX, iWidthY ) )
                    {
                        ...
                        {
#ifdef _LOGOUT
                            logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "NearBuildings() failed " );
#endif
                            continue;
                        }

                        // use the game's opinion of building this building
                        if ( theMap.FoundationCost( hex, iBldg, 0, (CVehicle const*)-1 ) < 0 )
                        {
#ifdef _LOGOUT
                            logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "FoundationCost() failed " );
#endif
                            continue;
                        }
                    ...
                    {
#ifdef _LOGOUT
                        logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "AreHexesOpen() failed " );
#endif
                        continue;
                    }
                }
...
//
void CAIMapUtil::FindHexOnTerrain( int iBldg, int iWidthX, int iWidthY, CHexCoord& hexFound )
{
#ifdef _LOGOUT
    logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "\nFindHexOnTerrain() player %d for building %d from %d,%d", m_iPlayer,
               iBldg, hexFound.X( ), hexFound.Y( ) );
#endif
    ...
    WORD wTest = MSW_ROAD | MSW_RESOURCE | MSW_OPFOR_VEHICLE | MSW_OPFOR_BUILDING | MSW_AI_VEHICLE | MSW_AI_BUILDING |
                 MSW_STAGING;
    ...
    while ( iStep < pGameData->m_iHexPerBlk )
    {
        ...
        for ( iY = 0; iY < iDeltay; ++iY )
        {
            ...
            for ( iX = 0; iX < iDeltax; ++iX )
            {
                ...
                // just want the borders of the area, which is
                // the newly expanded to hexes on the edge of area
                if ( hex.X( ) != hcStart.X( ) && hex.X( ) != hcEnd.X( ) && hex.Y( ) != hcStart.Y( ) &&
                     hex.Y( ) != hcEnd.Y( ) )
                    continue;
                ...
                */
...
#ifdef _LOGOUT
                    logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC,
                               "CAIMapUtil::FindHexOnTerrain() testing %d,%d which is %d terrain ", hex.X( ), hex.Y( ),
                               (int)pGameHex->GetType( ) );
#endif
                    iRating = 0;
...
#ifdef _LOGOUT
                        logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC,
                                   "CAIMapUtil::FindHexOnTerrain() testing %d,%d has rating %d terrain ", hex.X( ),
                                   hex.Y( ), iRating );
#endif
                        if ( iRating < 3 )
                            continue;
...
#ifdef _LOGOUT
                            logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC,
                                       "CAIMapUtil::FindHexOnTerrain() testing %d,%d too near buildings ", hex.X( ),
                                       hex.Y( ) );
#endif
                            continue;
                        }
                ...
                if ( !AreHexesOpen( MAX_ADJACENT, hex, iWidthX, iWidthY ) )
                {
                    ...
                    {
                        logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC,
                                   "CAIMapUtil::FindHexOnTerrain() testing %d,%d AreHexesOpen failed ", hex.X( ),
                                   hex.Y( ) );
                    }
#endif
                    continue;
                ...
                if ( pGameData->m_iSmart && pGameData->m_iHexPerBlk > 32 )
                {
                    if ( !IsOutOpforRange( hex ) )
                        continue;
                }
                ...
                if ( !GetPathRating( hexFound, hex ) )
                {
                    ...
                    {
                        logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC,
                                   "CAIMapUtil::FindHexOnTerrain() testing %d,%d GetPathRating failed ", hex.X( ),
                                   hex.Y( ) );
                    }
#endif
                    continue;
                ...
                iRating = GetBufferRating( iBldg, hex, iWidthX, iWidthY );
        ...
        if ( iBestHex < m_iMapSize )
        {
            ...
            logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC,
                       "CAIMapUtil::FindHexOnTerrain() player %d found %d,%d  which is %d terrain", m_iPlayer,
                       hexFound.X( ), hexFound.Y( ), (int)pGameHex->GetType( ) );
#endif
            return;
        }
...
#ifdef _LOGOUT
    logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "FindHexOnTerrain() player %d for building %d ", m_iPlayer, iBldg );
    logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "second try with iDelta=%d,%d iStep=%d ", iDeltax, iDeltay, iStep );
#endif
...
#ifdef _LOGOUT
            logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "CAIMapUtil::FindHexOnTerrain() player %d finally found %d,%d ",
...
#ifdef _LOGOUT
    logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "CAIMapUtil::FindHexOnTerrain() player %d no hex found ", m_iPlayer );
#endif
}
...
#ifdef _LOGOUT
        logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "CAIMapUtil::FindAssaultHex() player %d from %d,%d to %d,%d ",
...
#ifdef _LOGOUT
                logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC,
                           "CAIMapUtil::FindAssaultHex() player %d found hex %d,%d for %d terrain ", m_iPlayer,
                           hex.X( ), hex.Y( ), iTerrain );
#endif
                hexTarget = hex;
                return;
            }
...
#ifdef _LOGOUT
    logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC,
               "CAIMapUtil::FindAssaultHex() player %d no hex found for %d terrain from %d,%d ", m_iPlayer, iTerrain,
               hexTarget.X( ), hexTarget.Y( ) );
#endif
}
    ...
    while ( iStep < pGameData->m_iHexPerBlk )
    {
        ...
        for ( iY = 0; iY < iDeltay; ++iY )
        {
            ...
            for ( iX = 0; iX < iDeltax; ++iX )
            {
                ...
                // just want the borders of the area, which is
                // the newly expanded to hexes on the edge of area
                if ( hexBridge.X( ) != hcFrom.X( ) && hexBridge.X( ) != hcTo.X( ) && hexBridge.Y( ) != hcFrom.Y( ) &&
                     hexBridge.Y( ) != hcTo.Y( ) )
                    continue;
    ...
    while ( iStep < pGameData->m_iHexPerBlk )
    {
        ...
        for ( iY = 0; iY < iDeltay; ++iY )
        {
            ...
            for ( iX = 0; iX < iDeltax; ++iX )
            {
                ...
                // just want the borders of the area, which is
                // the newly expanded to hexes on the edge of area
                if ( hexRoad.X( ) != hcFrom.X( ) && hexRoad.X( ) != hcTo.X( ) && hexRoad.Y( ) != hcFrom.Y( ) &&
                     hexRoad.Y( ) != hcTo.Y( ) )
                    continue;
                ...
                if ( ( m_pMap[i] & MSW_PLANNED_ROAD ) )
                {
                    if ( ( m_pMap[i] & MSW_AI_BUILDING ) || ( m_pMap[i] & MSW_OPFOR_BUILDING ) )
                        continue;
                    ...
                    if ( m_pMap[j] & MSW_ROAD || m_pMap[j] & MSW_AI_BUILDING )
                        bRoads = TRUE;
...
#ifdef _LOGOUT
        logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "Found road site player %d at %d,%d ", m_iPlayer, hexFound.X( ),
                   hexFound.Y( ) );
#endif
    }
}
    ...
    while ( pos != NULL )
    {
        ...
        if ( pUnit != NULL )
        {
            ...
            if ( pUnit->GetTypeUnit( ) == CStructureData::rocket )
            {
                ...
                iDist = pGameData->GetRangeDistance( hexNear, hexRocket );
...
//
int CAIMapUtil::GetBufferRating( int iBldg, CHexCoord& hexRate, int iWidthX, int iWidthY )
{
    CHexCoord hcStart, hcEnd, hex;
    ...
    for ( int iY = 0; iY < iDeltaY; ++iY )
    {
        ...
        for ( int iX = 0; iX < iDeltaX; ++iX )
        {
            ...
            if ( iTerrain == CHex::ocean || iTerrain == CHex::river || iTerrain == CHex::lake ||
                 iTerrain == CHex::road || iTerrain == (BYTE)0xFF )
                continue;

            // some terrain just discourages farming
            if ( iTerrain == CHex::desert || iTerrain == CHex::hill || iTerrain == CHex::mountain ||
                 iTerrain == CHex::swamp )
            {
                if ( iRating )
                    iRating--;
            ...
            CTerrainData const* pTerrain = pGameData->GetTerrainData( iTerrain );
            if ( pTerrain == NULL )
                continue;

            // accumulate multiplier based on type indicated
            if ( iBldg == CStructureData::lumber && iTerrain == CHex::forest )
                iRating++;
            else if ( iBldg == CStructureData::lumber && iTerrain != CHex::forest )
                iRating += 0;
            // get farm multiplier
...
//
int CAIMapUtil::GetClosestTo( CHexCoord hexFrom, int iBldg1, int iBldg2, int iBldg3 )
{
        ...
        if ( pUnit != NULL )
        {
            ...
            if ( pUnit->GetType( ) == CUnit::building )
            {
                ...
                if ( pCopyCBuilding != NULL )
                {
                    ...
                    // and family type
                    iBldgType = pCopyCBuilding->m_aiDataOut[CAI_BUILDTYPE];

                    // go thru possible building types for testing
                    for ( int i = 0; i < 3; ++i )
                    {
                        // is this a valid type for testing?
                        if ( aiBldg[i] > 0 && aiBldg[i] < CStructureData::num_types )
                        {
                            // is this a member of the group of type of
                            // the building we want?
                            if ( iBldgType == aiBldg[i] )
                            {
                                iDist = pGameData->GetRangeDistance( hexFrom, hexTo );
...
BOOL CAIMapUtil::GetPathRating( CHexCoord& hexFrom, CHexCoord& hexTo, int iVehType /*= CTransportData::construction*/ )
{
    ...
    return ( thePathMap.GetPath( hexFrom, hexTo, 0, 0, m_pMap, iVehType ) );
...
int CAIMapUtil::GetEnterableHexes( CHexCoord hexStart, CHexCoord hexEnd )
{
    ...
    logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "in area of %d,%d to %d,%d ", hexStart.X( ), hexStart.Y( ), hexEnd.X( ),
               hexEnd.Y( ) );
#endif
...
#ifdef _LOGOUT
    logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "count=%d \n", iCnt );
#endif
...
BOOL CAIMapUtil::GetClosestWaterHex( CHexCoord& hcFrom, CHexCoord& hcTo )
{
    ...
    if ( hcBase.X( ) < iHalf && hcWater.X( ) >= iHalf )
    {
        hcFrom.X( hcWater.Wrap( hcWater.X( ) - iDist ) );
        hcTo.X( hcBase.X( ) );
    }
    else if ( hcBase.X( ) >= iHalf && hcWater.X( ) < iHalf )
    {
        hcFrom.X( hcBase.X( ) );
        hcTo.X( hcWater.Wrap( hcWater.X( ) + iDist ) );
    }
    else if ( ( hcBase.X( ) >= iHalf && hcWater.X( ) >= iHalf ) || ( hcBase.X( ) <= iHalf && hcWater.X( ) <= iHalf ) )
    {
    ...
    iHalf = (int)( m_wRows / 2 );
    if ( hcBase.Y( ) < iHalf && hcWater.Y( ) >= iHalf )
    {
        hcFrom.Y( hcWater.Wrap( hcWater.Y( ) - iDist ) );
        hcTo.Y( hcBase.Y( ) );
    }
    else if ( hcBase.Y( ) >= iHalf && hcWater.Y( ) < iHalf )
    {
        hcFrom.Y( hcBase.Y( ) );
        hcTo.Y( hcWater.Wrap( hcWater.Y( ) + iDist ) );
    }
    else if ( ( hcBase.Y( ) >= iHalf && hcWater.Y( ) >= iHalf ) || ( hcBase.Y( ) <= iHalf && hcWater.Y( ) <= iHalf ) )
    {
...
//
void CAIMapUtil::FindHexByWater( int iBldg, int iWidthX, int iWidthY, CHexCoord& hexFound )
{
    CStructureData const* pBldgData = pGameData->GetStructureData( iBldg );
    if ( pBldgData == NULL )
        return;
...
TryTryAgain:
    ...
    logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "from %d,%d to %d,%d \n", hcFrom.X( ), hcFrom.Y( ), hcTo.X( ), hcTo.Y( ) );
#endif
...
#ifdef _LOGOUT
                        logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC,
                                   "FindHexByWater() FoundationCost direction %d at %d,%d passed", j, hcTo.X( ),
                                   hcTo.Y( ) );
#endif
                    }
                    // store rating
    ...
    while ( pos != NULL )
    {
        ...
        if ( pUnit != NULL )
        {
            ...
            if ( pUnit->GetType( ) == CUnit::building )
            {
                ...
                if ( pUnit->GetTypeUnit( ) == iNearBldg || pUnit->GetTypeUnit( ) == iThisBldg )
                {
                    EnterCriticalSection( &cs );
                    CBuilding* pBldg = theBuildingMap.GetBldg( pUnit->GetID( ) );
                    if ( pBldg != NULL )
...
BOOL CAIMapUtil::IsHexInCity( CHexCoord& hexTest )
{
    ...
    if ( i < m_iMapSize )
    {
        ...
        if ( !AreHexesOpen( MAX_ADJACENT, hexTest, m_iWidth, m_iHeight ) )
            return FALSE;
        ...
        // use the game's opinion of building this building
        if ( theMap.FoundationCost( hexTest, m_iBldg, 0, (CVehicle const*)-1 ) < 0 )
            return FALSE;
...
// within the scope of the city
//
void CAIMapUtil::FindHexInCity( int iBldg, int iWidthX, int iWidthY, CHexCoord& hexFound )
{

#ifdef _LOGOUT
    logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "CAIMapUtil::FindHexInCity() player %d for a %d ", m_iPlayer, iBldg );
#endif

    // for vehicle factories, need to check access for all
    // wheel types
    BOOL bVehFactory = FALSE;
    if ( iBldg == CStructureData::heavy || iBldg == CStructureData::light_0 || iBldg == CStructureData::light_1 ||
         iBldg == CStructureData::light_2 )
        bVehFactory = TRUE;

    // check for these damn, worthless buildings
    BOOL bCitizen = FALSE;
    if ( iBldg >= CStructureData::apartment_1_1 && iBldg <= CStructureData::office_3_2 )
        bCitizen = TRUE;
    ...
    WORD      wTest = MSW_STAGING | MSW_ROAD | MSW_RESOURCE | MSW_AI_BUILDING | MSW_OPFOR_BUILDING;
    ...
    while ( iStep < pGameData->m_iHexPerBlk )
    {
...
#ifdef _LOGOUT
        logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "CAIMapUtil::FindHexInCity() player %d from %d,%d to %d,%d  iStep=%d ",
        ...
        for ( int iY = 0; iY < iDeltay; ++iY )
        {
            ...
            for ( int iX = 0; iX < iDeltax; ++iX )
            {
                ...
                // just want the borders of the area, which is
                // the newly expanded to hexes on the edge of area
                if ( hex.X( ) != hcFrom.X( ) && hex.X( ) != hcTo.X( ) && hex.Y( ) != hcFrom.Y( ) &&
                     hex.Y( ) != hcTo.Y( ) )
                    continue;
...
#ifdef _LOGOUT
                    logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "CAIMapUtil::FindHexInCity() player %d found %d,%d \n",
                               m_iPlayer, hexFound.X( ), hexFound.Y( ) );
#endif
                    return;
                }
...
#ifdef _LOGOUT
    logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "CAIMapUtil::FindHexInCity() not found \n" );
#endif
}
#endif
...
//
void CAIMapUtil::GetExploreHex( CAIUnit* pUnit, CHexCoord& hexFound, CHexCoord& hcNow )
{
...
#ifdef _LOGOUT
        logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "GetExploreHex() player %d unit %ld going to %d,%d \n", m_iPlayer,
                   pUnit->GetID( ), hexFound.X( ), hexFound.Y( ) );
#endif
        return;
    }
    ...
    hexFound = hcNow;
    FindStagingHex( hcNow, iStep, iStep, pVehData->GetType( ), hexFound, FALSE );
}

//
// return a 0 if adjacent to a road, else iRating if not
//
int CAIMapUtil::RateNearRoad( CAIHex* paiHex, int iRating, int iWidthX, int iWidthY )
{
    int iCells[MAX_ADJACENT];
    for ( int i = 0; i < MAX_ADJACENT; ++i ) iCells[i] = 0;
    ...
    {
        for ( int i = 0; i < MAX_ADJACENT; ++i ) iCells[i] = 1;
    }
    ...
    for ( int i = 0; i < MAX_ADJACENT; ++i )
    {
        ...
        if ( iCells[i] )
        {
            ...
            if ( ( wStatus & MSW_ROAD ) || ( wStatus & MSW_PLANNED_ROAD ) )
                return ( 0 );
        }
    }
    return ( iRating );
        ...
        if ( pUnit != NULL )
        {
            ...
            if ( pUnit->GetType( ) == CUnit::building )
            {
                ...
                if ( pCopyCBuilding != NULL )
                {
                    ...
                    // is this a member of the group of type of
                    // the building we want?
                    if ( pCopyCBuilding->m_aiDataOut[CAI_BUILDTYPE] == iNext )
                    {
                        iDist = pGameData->GetRangeDistance( hFrom, hTo );
                        if ( iDist < m_iBufferSize )
                            continue;
                        if ( iDist < iNextDist )
                            iNextDist = iDist;
                    }
                    else if ( pCopyCBuilding->m_aiDataOut[CAI_BUILDTYPE] == iOther )
                    {
                        iDist = pGameData->GetRangeDistance( hFrom, hTo );
                        if ( iDist < m_iBufferSize )
                            continue;
...
BOOL CAIMapUtil::FindBldgScope( CRect* prScope, int iBldg )
{
    // load the building family types into an array for processing
    int aiBldg[3];
    for ( int i = 0; i < 3; ++i ) aiBldg[i] = CStructureData::num_types;
                ...
                if ( pCopyCBuilding != NULL )
                {
                    ...
                    for ( int i = 0; i < 3; ++i )
                    {
                        ...
                        if ( aiBldg[i] < CStructureData::num_types )
                        {
                            // is this a member of the group of type of
                            // the building we want?
                            if ( aiBldg[i] == pCopyCBuilding->m_aiDataOut[CAI_BUILDTYPE] )
                            {
                                ...
                                iSX = ( ix - pBldgData->GetCX( ) ) - m_iBufferSize;
                                // if( iSX < m_wBaseCol )
                                //	iSX = m_wBaseCol;
                                ...
                                iSY = ( iy - pBldgData->GetCY( ) ) - m_iBufferSize;
                                // if( iSY < m_wBaseRow )
                                //	iSY = m_wBaseRow;
                                ...
                                iEX = ( ix + pBldgData->GetCX( ) ) + m_iBufferSize;
                                // if( iEX > m_wEndCol )
                                //	iEX = m_wEndCol;
                                ...
                                iEY = ( iy + pBldgData->GetCY( ) ) + m_iBufferSize;
                                // if( iEY > m_wEndRow )
                                //	iEY = m_wEndRow;
...
#ifdef _LOGOUT
    logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "\nBuiding Scope is: " );
    logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "From %d,%d to %d,%d \n", prScope->left, prScope->top, prScope->right,
               prScope->bottom );
#endif
...
//
void CAIMapUtil::GetWaterStagingArea( int iShipType, CHexCoord& hcStart, CHexCoord& hcEnd, int iWidth, int iHeight )
{
    CTransportData const* pShipData = pGameData->GetTransportData( iShipType );
    if ( pShipData == NULL )
        return;
...
#ifdef _LOGOUT
                        logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC,
                                   "GetWaterStagingArea() player %d shipyard %ld at %d,%d ", m_iPlayer, pUnit->GetID( ),
                                   hcShipYard.X( ), hcShipYard.Y( ) );
#endif
                    }
                }
            }
...
#ifdef _LOGOUT
    logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "GetWaterStagingArea() player %d from %d,%d to %d,%d \n", m_iPlayer,
               hcStart.X( ), hcStart.Y( ), hcEnd.X( ), hcEnd.Y( ) );
#endif
}

void CAIMapUtil::ExpandAreaWithHex( CHexCoord& hcStart, CHexCoord& hcEnd, CHexCoord& hex )
{
    ...
    {
        if ( hex.X( ) < hcStart.X( ) && hex.X( ) > hcEnd.X( ) )
        {
            if ( abs( hex.Diff( hcStart.X( ) - hex.X( ) ) ) < abs( hex.Diff( hex.X( ) - hcEnd.X( ) ) ) )
                hcStart.X( hex.X( ) );
            else
                hcEnd.X( hex.X( ) );
        }
    ...
    {
        if ( hex.Y( ) < hcStart.Y( ) && hex.Y( ) > hcEnd.Y( ) )
        {
            if ( abs( hex.Diff( hcStart.Y( ) - hex.Y( ) ) ) < abs( hex.Diff( hex.Y( ) - hcEnd.Y( ) ) ) )
                hcStart.Y( hex.Y( ) );
            else
                hcEnd.Y( hex.Y( ) );
        }
...
//
void CAIMapUtil::FindInfStagingArea( CHexCoord& hexBldg, CHexCoord& hcFrom, CHexCoord& hcTo )
{
    ...
    logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "based on %d,%d with barracks at %d,%d ", hexBase.X( ), hexBase.Y( ),
               hexBldg.X( ), hexBldg.Y( ) );
    logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "and staging from %d,%d to %d,%d ", hcFrom.X( ), hcFrom.Y( ), hcTo.X( ),
               hcTo.Y( ) );
    logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "city bounds are %d,%d to %d,%d \n", m_iCitySX, m_iCitySY, m_iCityEX,
               m_iCityEY );
#endif
}
...
//
BOOL CAIMapUtil::GetStagingAreaNear( CHexCoord& hexNear, CHexCoord& hcStart, CHexCoord& hcEnd, int iWidth, int iHeight,
                                     int iType /*= CTransportData::combat*/ )
{
    ...
    */
    ...
    if ( iMaxStep < iStep || iMaxStep >= pGameData->m_iHexPerBlk )
        iMaxStep = ( iStep * 3 );
...
#ifdef _LOGOUT
    logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "\nGetStagingAreaNear() player %d staging near %d,%d ", m_iPlayer,
               hexNear.X( ), hexNear.Y( ) );
    logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "GetStagingAreaNear() for iWidth=%d iHeight=%d iType=%d ", iWidth, iHeight,
               iType );
    logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "GetStagingAreaNear() for iStep=%d iMaxStep=%d iBestCnt=%d ", iStep,
               iMaxStep, iBestCnt );
#endif
    ...
    while ( iStep < iMaxStep )
    {
...
#ifdef _LOGOUT
        logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "GetStagingAreaNear() player %d considering %d,%d to %d,%d ", m_iPlayer,
        ...
        for ( iY = 0; iY < iDeltaY; ++iY )
        {
            ...
            for ( iX = 0; iX < iDeltaX; ++iX )
            {
                ...
                // just want the borders of the area, which is
                // the newly expanded to hexes on the edge of area
                if ( hex.X( ) != hexStart.X( ) && hex.X( ) != hexEnd.X( ) && hex.Y( ) != hexStart.Y( ) &&
                     hex.Y( ) != hexEnd.Y( ) )
                    continue;
...
#ifdef _LOGOUT
            logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "player %d selected area of %d,%d to %d,%d \n", m_iPlayer,
                       hcStart.X( ), hcStart.Y( ), hcEnd.X( ), hcEnd.Y( ) );
#endif
            return ( TRUE );
        }
...
//
int CAIMapUtil::GetStagingCount( CHexCoord& hexStart, int iWidth, int iHeight, int iType )
{
    int       iCnt = 0;
    ...
    WORD wTest = MSW_AI_VEHICLE | MSW_OPFOR_VEHICLE | MSW_STAGING | MSW_RESOURCE | MSW_AI_BUILDING | MSW_OPFOR_BUILDING;
...
//
void CAIMapUtil::GetStagingArea( CHexCoord& hcStart, CHexCoord& hcEnd, int iWidth, int iHeight )
{
    ...
    logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "starting area of %d,%d to %d,%d ", hcStart.X( ), hcStart.Y( ), hcEnd.X( ),
               hcEnd.Y( ) );
    logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "city bounds are %d,%d to %d,%d \n", m_iCitySX, m_iCitySY, m_iCityEX,
               m_iCityEY );
#endif
    ...
    {
        // get midpoint of this side
        int i = abs( hcCity.Diff( hcEnd.X( ) - hcStart.X( ) ) ) / 2;
    ...
    {
        // get midpoint of this side
        int i = abs( hcCity.Diff( hcEnd.Y( ) - hcStart.Y( ) ) ) / 2;
        hcCity.X( hcEnd.X( ) );
    ...
    {
        // get midpoint of this side
        int i = ( hcCity.Diff( hcEnd.Y( ) - hcStart.Y( ) ) ) / 2;
        hcCity.X( hcStart.X( ) );
        hcCity.Y( hcCity.Wrap( hcStart.Y( ) + i ) );
...
//
void CAIMapUtil::FlagStagingArea( BOOL bFlag, int iSX, int iSY, int iEX, int iEY )
{
                                 ...
                                 BOOL bFindPath /*=FALSE*/ )
{
    ...
    if ( iVehType == CTransportData::landing_craft || iVehType == CTransportData::infantry_carrier )
    {
    ...
    logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "within area of %d,%d to %d,%d ", hexStart.X( ), hexStart.Y( ), hexEnd.X( ),
               hexEnd.Y( ) );
#endif

    WORD wTest = MSW_ROAD | MSW_PLANNED_ROAD | MSW_RESOURCE | MSW_AI_VEHICLE | MSW_AI_BUILDING;
...
#ifdef _LOGOUT
        dwEnd = timeGetTime( );
        logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "player %d staging at %d,%d  took %ld ticks \n", m_iPlayer,
...
#ifdef _LOGOUT
    logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "FindStagingHex() trying all hexes " );
    logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "iDeltaX=%d iDeltaY=%d  starting from %d,%d ", iDeltaX, iDeltaY, iSX, iSY );
#endif
    ...
    for ( y = 0; y < iDeltaY; ++y )
    {
        for ( x = 0; x < iDeltaX; ++x )
        {
...
#ifdef _LOGOUT
            logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "at %d,%d terrain is %d  GetUnits() is %d ", hex.X( ), hex.Y( ),
                       pGameHex->GetType( ), (int)pGameHex->GetUnits( ) );
#endif
            ...
            {
#ifdef _LOGOUT
                logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "invalid offset %d ", i );
#endif
                continue;
            }
            ...
            {
#ifdef _LOGOUT
                logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "m_pMap[i] = %d ", m_pMap[i] );
#endif
                continue;
            }
...
#ifdef _LOGOUT
    dwEnd = timeGetTime( );
    logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "staging failed after %ld ticks \n", ( dwEnd - dwStart ) );
#endif
}
...
//
void CAIMapUtil::FindStagingHex( CHexCoord& hexNearBy, int iWidth, int iHeight, int iVehType, CHexCoord& hexDest,
                                 BOOL bExclude /*=TRUE*/ )
{
    // get pointer to vehicle type data
    CTransportData const* pVehData = pGameData->GetTransportData( iVehType );
...
TryTryAgain:
    ...
    if ( iDeltaY > (int)m_wRows )
        iDeltaY = m_wRows;
    if ( iDeltaX == m_wCols || iDeltaY == m_wRows )
        iRadi = 10;
    ...
    if ( bExclude )
    {
        logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "excluding area of %d,%d to %d,%d ", hcStartExclude.X( ),
                   hcStartExclude.Y( ), hcEndExclude.X( ), hcEndExclude.Y( ) );
    }
#endif
    ...
    while ( iCnt )
    {
        ...
        if ( iVehType == CTransportData::construction || iVehType == CTransportData::heavy_truck )
        {
            if ( !IsOutOpforRange( hex ) )
                continue;
        }
    ...
    while ( iCnt < pGameData->m_iHexPerBlk )
    {
        ...
        for ( iY = 0; iY < iDeltaY; ++iY )
        {
            ...
            for ( iX = 0; iX < iDeltaX; ++iX )
            {
                ...
                // just want the borders of the area, which is
                // the newly expanded to hexes on the edge of area
                if ( hex.X( ) != hcStartArea.X( ) && hex.X( ) != hcEndArea.X( ) && hex.Y( ) != hcStartArea.Y( ) &&
                     hex.Y( ) != hcEndArea.Y( ) )
                    continue;
...
void CAIMapUtil::FindLandingHex( CHexCoord& hexHead )
{
    ...
    // get pointer to vehicle type data
    CTransportData const* pVehData = pGameData->GetTransportData( CTransportData::landing_craft );
    if ( pVehData == NULL )
        return;
    ...
    while ( iCnt < iLimit )
    {
        ...
        for ( iY = 0; iY < iDeltaY; ++iY )
        {
            ...
            for ( iX = 0; iX < iDeltaX; ++iX )
            {
                ...
                if ( !GetPathRating( hexVeh, hex, CTransportData::landing_craft ) )
                    continue;
...
//
BOOL CAIMapUtil::IsHexInArea( CHexCoord& hcStart, CHexCoord& hcEnd, CHexCoord& hex )
{
...
// is possible
//
void CAIMapUtil::FindRetreatHex( CAIUnit* pFleeUnit, CHexCoord& hexFlee, CAIUnit* pAttacker, CHexCoord& hexAttacking )
{
    ...
    // get pointer to vehicle type data for CanTravelHex()
    CTransportData const* pVehData = pGameData->GetTransportData( pFleeUnit->GetTypeUnit( ) );
    if ( pVehData == NULL )
        return;
    ...
    for ( iY = 0; iY < iDeltaY; ++iY )
    {
        ...
        for ( iX = 0; iX < iDeltaX; ++iX )
        {
            ...
            CHex*               pHex     = theMap.GetHex( hex );
            CTerrainData const* pTerrain = pGameData->GetTerrainData( (int)pHex->GetType( ) );
            if ( pTerrain == NULL )
                continue;
            ...
            if ( iRange )
            {
                m_pwaWork[i] = ( pTerrain->GetDefenseMult( pVehData->GetWheelType( ) ) * iArea ) / iRange;
            }
        }
    }
...
//
void CAIMapUtil::FindApproachHex( CHexCoord& hexTarget, CAIUnit* pUnit, CHexCoord& hexDest )
{
    ...
    int iRangeToTarget = pGameData->GetRangeDistance( hexDest, hexTarget ) - ( iSpotting / 2 );
    ...
    for ( iy = 0; iy < iDeltaY; iy++ )
    {
        ...
        for ( ix = 0; ix < iDeltaX; ix++ )
        {
            ...
            if ( hexAt.X( ) != hexStart.X( ) && hexAt.X( ) != hexEnd.X( ) && hexAt.Y( ) != hexStart.Y( ) &&
                 hexAt.Y( ) != hexEnd.Y( ) )
                continue;
    ...
    for ( iy = 0; iy < iDeltaY; iy++ )
    {
        ...
        for ( ix = 0; ix < iDeltaX; ix++ )
        {
            ...
            CTerrainData const* pTerrain = pGameData->GetTerrainData( (int)pGameHex->GetType( ) );
            if ( pTerrain == NULL )
                continue;

            m_pwaWork[i] = pTerrain->GetDefenseMult( pVehData->GetWheelType( ) );
                                 ...
                                 CTransportData const* pVehData )
{
    ...
    for ( iy = 0; iy < iDeltaY; iy++ )
    {
        ...
        for ( ix = 0; ix < iDeltaX; ix++ )
        {
            ...
            {
                if ( hcAt.X( ) != hcStart.X( ) && hcAt.X( ) != hcEnd.X( ) && hcAt.Y( ) != hcStart.Y( ) &&
                     hcAt.Y( ) != hcEnd.Y( ) )
                    continue;
            }
...
#ifdef _LOGOUT
    logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "CAIMapUtil::IsLandingArea() at %d,%d water=%d land=%d ", hexLC.X( ),
               hexLC.Y( ), wCntWater, wCntLand );
#endif

    if ( wCntWater >= 1 && wCntLand >= 1 )
        return TRUE;
...
//
BOOL CAIMapUtil::IsWaterAdjacent( CHexCoord hexWater, int iWidthX, int iWidthY )
{
    ClearGroupHexes( );
...
BOOL CAIMapUtil::IsCitizenTerrain( CHexCoord& hcStart, int iWidthX, int iWidthY )
{
    ...
    if ( pHex->GetType( ) != CHex::rough && pHex->GetType( ) != CHex::hill && pHex->GetType( ) != CHex::swamp &&
    ...
    WORD wTest = MSW_AI_BUILDING | MSW_OPFOR_BUILDING | MSW_RESOURCE | MSW_STAGING;
...
int CAIMapUtil::GetMineralRating( CHexCoord& hexRate, int iWidthX, int iWidthY, int iMaterial )
{
    CHexCoord hcStart, hcEnd, hex;
    hcStart.X( hexRate.X( ) );
    hcStart.Y( hexRate.Y( ) );
...
//
BOOL CAIMapUtil::NearBuildings( CHexCoord& hexRate, int iWidthX, int iWidthY, BOOL bAllowResc )
{
    CHexCoord hcStart, hcEnd, hex;
...
//
BOOL CAIMapUtil::AreHexesOpen( int iDirection, CHexCoord& hexRate, int iWidthX, int iWidthY )
{
    ClearGroupHexes( );
    ...
    int i = GetMapOffset( hexRate.X( ), hexRate.Y( ) );
    if ( i < m_iMapSize )
    {
        m_pwaWork[i] = 1;
        return TRUE;
...
//
int CAIMapUtil::GetMultiplier( CHexCoord hexBuild, int iWidthX, int iWidthY, int iType )
{
    ClearGroupHexes( );
    ...
    for ( int i = 0; i < m_iGrpSize; ++i )
    {
        ...
        CTerrainData const* pTerrain = pGameData->GetTerrainData( pGameHex->GetType( ) );
...
BOOL CAIMapUtil::IsHexValidToBuild( CHexCoord& hex )
{
    ...
    if ( iTerrain == CHex::ocean || iTerrain == CHex::river || iTerrain == CHex::road || iTerrain == CHex::lake )
        return FALSE;
    ...
    WORD wTest = 0;
    wTest      = MSW_AI_BUILDING | MSW_OPFOR_BUILDING | MSW_AI_VEHICLE | MSW_OPFOR_VEHICLE | MSW_STAGING;
...
WORD CAIMapUtil::ConvertStatus( CAIHex* pHex, WORD wOldStatus )
{
    ...
    if ( wOldStatus & MSW_PLANNED_ROAD && !( wStatus & MSW_ROAD ) )
    ...
    int iOwner = 0;
    if ( pHex->m_dwUnitID > 0 && pHex->m_iUnit == CUnit::building )
    {
...
CAIMapUtil::CAIMapUtil( WORD* pMap, CAIUnitList* plUnits, int iBaseX, int iBaseY, WORD wBaseCol, WORD wBaseRow,
                        WORD wCols, WORD wRows, int iPlayer )
{
    m_bUnknown = TRUE;
    ...
    m_iGrpSize = pGameData->m_iStructureSize * pGameData->m_iStructureSize;
    try
    {
...
{
    logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "Status of map and roads, player %d from %d,%d to %d,%d ", m_iPlayer,
               m_wBaseCol, m_wBaseRow, m_wEndCol, m_wEndRow );
    ...
    for ( int iY = 0; iY < 64; ++iY )
    {
        ...
        logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "%s ", szLine );
    }
}
#endif
...
void CAIMapUtil::ReportGroupHex( int iBldg, int iWidthX, int iWidthY, CHexCoord& hex )
{
    ...
    for ( int i = 0; i < m_iGrpSize; ++i )
    {
        ...
        // get location from game data, and build multiplier
        pGameData->GetCHexData( paiGrpHex );
        CTerrainData const* pTerrain = pGameData->GetTerrainData( (int)paiGrpHex->m_cTerrain );

        sName = pTerrain->GetDesc( );

        logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "Hex %d,%d is %s map value = %d ", paiGrpHex->m_iX, paiGrpHex->m_iY,
        ...
        logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "Build Multi is %d, Farm Multi is %d ", pTerrain->GetBuildMult( ),
                   pTerrain->GetFarmMult( ) );
        ...
        {
            logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "Unit found: %ld ", paiGrpHex->m_dwUnitID );
        }
    }
    logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "Total Build Multiplier is %d \n", iMulti );
}
#endif
