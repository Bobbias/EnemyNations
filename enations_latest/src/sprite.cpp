...
CSpriteDIB::CalcBoundingRect() const {
    switch (m_iType) {
        case CSprite::EFFECT:
        case CSprite::STRUCTURE:
            return StructureCalcBoundingRect();
        case CSprite::TERRAIN:
            return TerrainCalcBoundingRect();
        case CSprite::VEHICLE:
            return VehicleCalcBoundingRect();
...
CSpriteDIB::IsHit(
    ...
    switch (m_iType) {
        case CSprite::EFFECT:
        case CSprite::STRUCTURE:
            return StructureIsHit(ptCursor);
        case CSprite::TERRAIN:
            return TerrainIsHit(ptCursor);
        case CSprite::VEHICLE:
            return VehicleIsHit(ptCursor);
    }
...
CSpriteDIB::TerrainIsHit(
    ...
    return xpanimatr->IsHexHit(hexcoord, ptCursor, rect);
}
...
CSpriteDIB::TerrainDraw() const {
    ...
    switch (iCursorMode) {
        ...
        case CHex::lousy_cur:
            iColor = 251;
            break;
        case CHex::land_exit_cur:
            iColor = 0;
            break;
        case CHex::sea_exit_cur:
            iColor = 252;
            break;
    }
...
//--------------------------------------------------------------------------
    ...
    CSpriteDIB::VehicleDraw(
        ...
        for (iY = iTopY; iY <= iBottomY; ++iY) {
            int fixX = Max( (int)((prVp->left << 16) + 0x00008000), *pfixLeft++);
            int iX = fixX >> 16;
            ...
            int fixRight = Min(*pfixRight++, (int)((prVp->right << 16) + 0x00008000));
