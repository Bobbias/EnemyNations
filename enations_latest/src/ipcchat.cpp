...
{
    CRect rect( LOWORD( uParam ), HIWORD( uParam ), LOWORD( lParam ), HIWORD( lParam ) );
...
void CChatWnd::OnMove( int x, int y )
{
    ...
    if ( rect.top > ( rectTB.top - GetSystemMetrics( SM_CYCAPTION ) ) )
    {
        ...
        PostMessage( IPC_UNHIDE_WINDOW, MAKEWPARAM( rect.left, rect.top ), MAKELPARAM( rect.right, rect.bottom ) );
    }
...
void CChatWnd::OnSelectplayer( )
{
    ...
    CComboBox* pPlayerBox = (CComboBox*)m_pdbChatBar->GetDlgItem( IDC_SELECTPLAYER );
    if ( pPlayerBox != NULL )
    {
...
void CChatWnd::OnChatIn( CMsgIPC* pMsg )
{
    ...
    else
    {
        ...
        if ( pMsg->m_iCC < sOld.GetLength( ) )
        {
            ...
            pChatView->GetEditCtrl( ).SetSel( pMsg->m_iCC, sOld.GetLength( ) );

            pChatView->GetEditCtrl( ).ReplaceSel( (LPCTSTR)pMsg->m_sMessage );
        ...
        pChatView->GetEditCtrl( ).SetSel( sNew.GetLength( ), sNew.GetLength( ), TRUE );
...
void CChatWnd::OnSize( UINT nType, int cx, int cy )
{
    ...
    {
        m_pdbChatBar->GetWindowRect( &rBar );
        m_pdbChatBar->SetWindowPos( &wndTop, 0, 0, cx, rBar.Height( ), SWP_SHOWWINDOW );
        RecalcLayout( );
    }
...
void CMySplitterWnd::OnChange( )
{
    ...
    if ( m_sLastSend.IsEmpty( ) || sOld.GetLength( ) < m_sLastSend.GetLength( ) )
    {
        msg.m_iCC      = 0;
        msg.m_sMessage = sOld;
        m_sLastSend    = sOld;
