...
void CBmBtnData::Init( CMmio* pMmio )
{
    ...
    m_pDib = new CDIB( ptrthebltformat->GetColorFormat( ), CBLTFormat::DIB_MEMORY, ptrthebltformat->GetMemDirection( ),
                       cx, cy );
}
...
{
    CTextColors textcolors( m_pTextBtnData->m_clr[1], m_pTextBtnData->m_clr[0], RGB( 50, 50, 50 ) );

    CGlobalSubClass::DrawButton( lpDIS,
                                 m_pTextBtnData->m_pcDib,  // small
                                 m_pTextBtnData->m_pcDib,  // large
                                 m_pBackDib, &m_pTextBtnData->m_fntText, textcolors );
...
void CTextButton::OnSize( UINT nType, int cx, int cy )
{
    ...
    //																			ptrthebltformat->GetDirection(), cx,
    //cy);
}
