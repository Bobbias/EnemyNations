...
#if defined(WIN32)
#include "vdmplay/tapiutil.h"
#ifdef WITH_DP
#include "dplay.h"
extern HRESULT(WINAPI *pDirectPlayCreate)(LPGUID lpGUID,
...
static void ParseIpxAddress(LPCSTR addr, LPSTR netNum, LPSTR nodeNum,
                            UINT &portNum) {
  char buf[64];
...
static void MakeIpxAddress(LPSTR addrBuf, LPCSTR netNum, LPCSTR nodeNum,
                           UINT portNum) {
  addrBuf[0] = 0;
...
// TCP/IP dialog box
BOOL CALLBACK EXPORT DlgProcIP(HWND hDlg, UINT uMsg, WPARAM wParam,
                               LPARAM lParam) {
  char sIPAddr[128];
  ...
  case WM_COMMAND:
    ...
    if (HIWORD(wParam) == BN_CLICKED)
      ...
      case IDOK: {
        ...
        sIPAddr[127] = 0;
        LPCSTR addrKey = GetWindowLong(hDlg, DWL_USER) ? "RegistrationAddress"
                                                       : "ServerAddress";
      ...
      case IDC_DIAG: {
        ...
        WinExec(serverMode ? "vpdiag /TCP /SERVER" : "vpdiag /TCP", SW_SHOW);
        return TRUE;
      }
      }
    break;
...
// IPX dialog box
BOOL CALLBACK EXPORT DlgProcIPX(HWND hDlg, UINT uMsg, WPARAM wParam,
                                LPARAM lParam) {
  ...
  case WM_COMMAND:
    ...
    if (HIWORD(wParam) == BN_CLICKED)
      ...
      case IDC_DIAG: {
        BOOL serverMode = GetWindowLong(hDlg, DWL_USER);
        WinExec(serverMode ? "vpdiag /ipx /SERVER" : "vpdiag /ipx", SW_SHOW);
        return TRUE;
      }
      }
    break;
...
// NETBIOS dialog box
BOOL CALLBACK EXPORT DlgProcNETBIOS(HWND hDlg, UINT uMsg, WPARAM wParam,
                                    LPARAM lParam) {
  ...
  case WM_INITDIALOG: {
    ...
    itoa(iLana, sBuf, 10);
    int iIndex = (int)SendMessage(hWnd, CB_FINDSTRINGEXACT, (WPARAM)-1,
                                  (LPARAM)(LPCSTR)sBuf);
    if (iIndex < 0)
      iIndex = 0;
  ...
  case WM_COMMAND:
    if (HIWORD(wParam) == BN_CLICKED)
      ...
      case IDC_DIAG: {
        BOOL serverMode = GetWindowLong(hDlg, DWL_USER);
        WinExec(serverMode ? "vpdiag /NETBIOS /SERVER" : "vpdiag /NETBIOS",
                SW_SHOW);
        return TRUE;
      }
      }
...
// COMM dialog box
BOOL CALLBACK EXPORT DlgProcCOMM(HWND hDlg, UINT uMsg, WPARAM wParam,
                                 LPARAM lParam) {
  ...
  case WM_INITDIALOG: {
    ...
    for (int iPort = 0; iPort < 255; iPort++) {
      ...
      HANDLE dev =
          CreateFile(sBuf, GENERIC_READ | GENERIC_WRITE, 0, NULL, OPEN_EXISTING,
    ...
    int iIndex =
        (int)SendMessage(hWnd, CB_FINDSTRINGEXACT, (WPARAM)-1, (LPARAM)sBuf);
    if (iIndex < 0)
      iIndex = 0;
    SendMessage(hWnd, CB_SETCURSEL, iIndex, 0);
...
BOOL CALLBACK EXPORT DlgProcMODEMSettings(HWND hDlg, UINT uMsg, WPARAM wParam,
                                          LPARAM lParam) {
...
// MODEM dialog box
BOOL CALLBACK EXPORT DlgProcMODEM(HWND hDlg, UINT uMsg, WPARAM wParam,
                                  LPARAM lParam) {
  ...
  case WM_COMMAND:
    if (HIWORD(wParam) == BN_CLICKED)
      ...
      case IDC_MODEM_ADV:
        DialogBox(vphInst, MAKEINTRESOURCE(IDD_ADV_MODEM_SETTINGS), hDlg,
                  (DLGPROC)DlgProcMODEMSettings);
        return (TRUE);
...
BOOL CALLBACK EXPORT DlgProcTAPI(HWND hDlg, UINT uMsg, WPARAM wParam,
                                 LPARAM lParam) {
  char sBuf[82];
  char fName[256];
  ...
  case WM_COMMAND:
    if (HIWORD(wParam) == BN_CLICKED) {
      CTapiConnection *tapiObj =
          (CTapiConnection *)GetWindowLong(hDlg, DWL_USER);
...
BOOL FAR PASCAL EnumDP(LPGUID lpGuid, LPSTR lpDesc, DWORD dwMajorVer,
                       DWORD dwMinorVer, LPVOID lpv) {
...
BOOL CALLBACK EXPORT DlgProcDP(HWND hDlg, UINT uMsg, WPARAM wParam,
                               LPARAM lParam) {
...
void doAdvDialog(HWND hWnd, int iProtocol, BOOL bServer) {
  ...
  switch (iProtocol) {
  case VPT_TCP:
    DialogBoxParam(hInst, MAKEINTRESOURCE(IDD_ADV_TCPIP), hWnd,
                   (DLGPROC)DlgProcIP, bServer);
    break;
  case VPT_IPX:
    DialogBoxParam(hInst, MAKEINTRESOURCE(IDD_ADV_IPX), hWnd,
                   (DLGPROC)DlgProcIPX, bServer);
    break;
  case VPT_NETBIOS:
    DialogBoxParam(hInst, MAKEINTRESOURCE(IDD_ADV_NETBIOS), hWnd,
                   (DLGPROC)DlgProcNETBIOS, bServer);
    break;
  case VPT_COMM:
    DialogBoxParam(hInst, MAKEINTRESOURCE(IDD_ADV_COMM), hWnd,
                   (DLGPROC)DlgProcCOMM, bServer);
    break;
  case VPT_MODEM:
    DialogBoxParam(hInst, MAKEINTRESOURCE(IDD_ADV_MODEM), hWnd,
                   (DLGPROC)DlgProcMODEM, bServer);
    break;
#if defined(WIN32)
  case VPT_TAPI:
    DialogBoxParam(hInst, MAKEINTRESOURCE(IDD_ADV_TAPI), hWnd,
                   (DLGPROC)DlgProcTAPI, bServer);
    break;
#ifdef WITH_DP
  case VPT_DP:
    DialogBoxParam(hInst, MAKEINTRESOURCE(IDD_ADV_DP), hWnd, (DLGPROC)DlgProcDP,
                   bServer);
    break;
#endif
#endif
