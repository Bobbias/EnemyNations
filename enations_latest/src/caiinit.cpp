...
CAIInitPos::CAIInitPos( int iPlayer, CAIMap* pMap, CAIUnitList* plUnits )
{
    m_iPlayer = iPlayer;
    m_pMap    = pMap;
    m_plUnits = plUnits;
...
void CAIInitPos::DoIt( void )
{
    ...
    */
    ...
    logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "\nPlaceRocket() player %d took %ld ticks at difficulty %d \n", m_iPlayer,
               ( dwEnd - dwStart ), pGameData->m_iSmart );

#endif

    const int iSup[]  = { CRaceDef::med_scout,   CRaceDef::med_tank, CRaceDef::light_tank,
                         CRaceDef::light_art,   CRaceDef::crane,    CRaceDef::heavy_truck,
                         CRaceDef::light_scout, CRaceDef::infantry, CRaceDef::infantry_carrier };
    const int iTyp[]  = { CTransportData::med_scout,   CTransportData::med_tank,     CTransportData::light_tank,
                         CTransportData::light_art,   CTransportData::construction, CTransportData::heavy_truck,
                         CTransportData::light_scout, CTransportData::infantry,     CTransportData::infantry_carrier };
    ...
    for ( i = 0; i < iCnt; ++i )
    {
        try
        {
            ...
            pUnit = new CAIUnit( theGame.GetID( ), m_iPlayer, CUnit::vehicle, CTransportData::med_scout );
            ASSERT_VALID( pUnit );
            m_plUnits->AddTail( (CObject*)pUnit );
        }
        catch ( CException e )
...
#ifdef _LOGOUT
        dwEnd = timeGetTime( );
        logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "PlaceVehicle() CTransportData::med_scout %ld took %ld ticks \n",
    ...
    for ( i = 0; i < iCnt; ++i )
    {
        ...
        {
            // CAIUnit( DWORD dwID, int iOwner );
            pUnit = new CAIUnit( theGame.GetID( ), m_iPlayer, CUnit::vehicle, CTransportData::med_tank );
            ASSERT_VALID( pUnit );
            m_plUnits->AddTail( (CObject*)pUnit );
        }
        catch ( CException e )
...
#ifdef _LOGOUT
        dwEnd = timeGetTime( );
        logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "PlaceVehicle() CTransportData::med_tank %ld took %ld ticks \n",
                   pUnit->GetID( ), ( dwEnd - dwStart ) );
#endif
    }
    ...
    for ( i = 0; i < iCnt; ++i )
    {
        ...
        {
            // CAIUnit( DWORD dwID, int iOwner );
            pUnit = new CAIUnit( theGame.GetID( ), m_iPlayer, CUnit::vehicle, CTransportData::light_tank );
            ASSERT_VALID( pUnit );
            m_plUnits->AddTail( (CObject*)pUnit );
        }
        catch ( CException e )
...
#ifdef _LOGOUT
        dwEnd = timeGetTime( );
        logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "PlaceVehicle() CTransportData::light_tank %ld took %ld ticks \n",
                   pUnit->GetID( ), ( dwEnd - dwStart ) );
#endif
    }
    ...
    for ( i = 0; i < iCnt; ++i )
    {
        ...
        {
            // CAIUnit( DWORD dwID, int iOwner );
            pUnit = new CAIUnit( theGame.GetID( ), m_iPlayer, CUnit::vehicle, CTransportData::light_art );
            ASSERT_VALID( pUnit );
            m_plUnits->AddTail( (CObject*)pUnit );
        }
        catch ( CException e )
    ...
    if ( pGameData->m_iSmart >= ( NUM_DIFFICUTY_LEVELS - 1 ) )
    {
        for ( i = 0; i < iCnt; ++i )
        {
            ...
            {
                // CAIUnit( DWORD dwID, int iOwner );
                pUnit = new CAIUnit( theGame.GetID( ), m_iPlayer, CUnit::vehicle, CTransportData::heavy_tank );
                ASSERT_VALID( pUnit );
                m_plUnits->AddTail( (CObject*)pUnit );
            }
            catch ( CException e )
    ...
    for ( i = 0; i < iCnt; ++i )
    {
        ...
        {
            // CAIUnit( DWORD dwID, int iOwner );
            pUnit = new CAIUnit( theGame.GetID( ), m_iPlayer, CUnit::vehicle, CTransportData::construction );
            ASSERT_VALID( pUnit );
            m_plUnits->AddTail( (CObject*)pUnit );
        }
        catch ( CException e )
...
#ifdef _LOGOUT
        dwEnd = timeGetTime( );
        logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "PlaceVehicle() CTransportData::construction %ld took %ld ticks \n",
                   pUnit->GetID( ), ( dwEnd - dwStart ) );
#endif
    }
    ...
    for ( i = 0; i < iCnt; ++i )
    {
        ...
        {
            // CAIUnit( DWORD dwID, int iOwner );
            pUnit = new CAIUnit( theGame.GetID( ), m_iPlayer, CUnit::vehicle, CTransportData::heavy_truck );
...
#ifdef _LOGOUT
        dwEnd = timeGetTime( );
        logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "PlaceVehicle() CTransportData::heavy_truck %ld took %ld ticks \n",
                   pUnit->GetID( ), ( dwEnd - dwStart ) );
#endif
    }
    ...
    */
...
#ifdef _LOGOUT
        dwEnd = timeGetTime( );
        logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "Place Scout %ld took %ld ticks \n", pUnit->GetID( ),
                   ( dwEnd - dwStart ) );
#endif
    }
    ...
    for ( i = 0; i < iCnt; ++i )
    {
        ...
        {
            // CAIUnit( DWORD dwID, int iOwner );
            pUnit = new CAIUnit( theGame.GetID( ), m_iPlayer, CUnit::vehicle, CTransportData::infantry );
...
#ifdef _LOGOUT
        dwEnd = timeGetTime( );
        logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "PlaceVehicle() CTransportData::infantry %ld took %ld ticks \n",
                   pUnit->GetID( ), ( dwEnd - dwStart ) );
#endif
    }
    ...
    for ( i = 0; i < iCnt; ++i )
    {
        ...
        {
            // CAIUnit( DWORD dwID, int iOwner );
            pUnit = new CAIUnit( theGame.GetID( ), m_iPlayer, CUnit::vehicle, CTransportData::infantry_carrier );
...
#ifdef _LOGOUT
        dwEnd = timeGetTime( );
        logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "PlaceVehicle() CTransportData::infantry_carrier %ld took %ld ticks \n",
                   pUnit->GetID( ), ( dwEnd - dwStart ) );
#endif
    }
...
#ifdef _LOGOUT
    logPrintf( LOG_PRI_ALWAYS, LOG_AI_MISC, "PlaceVehicle:: id=%ld type=%d at %d,%d ", pUnit->GetID( ), iVeh,
...
void CAIInitPos::PlaceRocket( void )
{
    ...
    {
        // CAIUnit( DWORD dwID, int iOwner );
        pUnit = new CAIUnit( theGame.GetID( ), m_iPlayer, CUnit::building, CStructureData::rocket );
        ASSERT_VALID( pUnit );
        m_plUnits->AddTail( (CObject*)pUnit );
    }
    catch ( CException* e )
    ...
    if ( m_hexRocket.X( ) > pGameData->m_iHexPerBlk )
        m_hexStart.X( ( m_hexRocket.X( ) / pGameData->m_iHexPerBlk ) * pGameData->m_iHexPerBlk );
    else
        m_hexStart.X( 0 );

    if ( m_hexRocket.Y( ) > pGameData->m_iHexPerBlk )
        m_hexStart.Y( ( m_hexRocket.Y( ) / pGameData->m_iHexPerBlk ) * pGameData->m_iHexPerBlk );
    else
        m_hexStart.Y( 0 );
