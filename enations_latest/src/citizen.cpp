...
CCitCon::CCitCon( int iBldg, CHexCoord const& hex, int iDir ): m_iBldg( iBldg ), m_hex( hex ), m_iDir( iDir )
{
...
void CPlayer::CitizenConstruction( )
{
    ...
    if ( ( m_iAptCap < m_iPplBldg + m_iPplVeh ) && ( m_iOfcCap < m_iPplBldg ) )
        bAptOk = RandNum( 1 );

    // do we need an apartment?
    if ( ( bAptOk ) && ( m_iAptCap < m_iPplBldg + m_iPplVeh ) )
    {
        int iBldg;
        if ( m_iPplBldg + m_iPplVeh < 5 * PPL_MUL )
            iBldg = CStructureData::apartment_base + RandNum( 1 );
        else if ( m_iPplBldg + m_iPplVeh < 15 * PPL_MUL )
    ...
    if ( m_iOfcCap < m_iPplBldg )
    {
        int iBldg;
        if ( m_iPplBldg + m_iPplVeh < 12 * PPL_MUL )
...
#if 0   // FIXME: What even is going on here?
...
static void RandDir( int* piDir )
{
    ...
    // set them
    for ( int iOn = 0; iOn <= MAX_NUM; iOn++ ) *( piDir + iOn ) = RandNum( MAX_NUM - iOn );
...
BOOL CPlayer::TryRoad( CCitCon* pCc )
{
    ...
    if ( pHex->GetType( ) == CHex::road )
        pCc->m_iLeft = 0;
    else if ( ( !pHex->IsWater( ) ) && ( !( pHex->GetUnits( ) & CHex::unit ) ) )
    {
    ...
    hexTry = HexNext( pCc, 0 );
    pHex   = theMap._GetHex( hexTry );
    if ( ( !pHex->IsWater( ) ) && ( pHex->GetType( ) != CHex::road ) && ( !( pHex->GetUnits( ) & CHex::unit ) ) )
    {
