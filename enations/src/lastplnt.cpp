...
#include <VersionHelpers.h> // NOTE: added to help determine windows version easier.
...
void CatchSE( SE_Exception e ) {
    ...
    case STATUS_STACK_OVERFLOW:
        ...
        break;
    default:
        _itoa( (int)e.m_pExCode, sNum2, 16 );
        break;
    }
    for ( int iOn = 0; iOn < 5; iOn++ )
        _itoa( e.m_stack[iOn], sNumS[iOn], 16 );
...
BOOL CConquerApp::InitInstance() {
    ...
    if ( iWinType != W32s ) {
        ...
        if ( lRtn == DISP_CHANGE_SUCCESSFUL ) {
            ...
            dev.dmPelsHeight = 480;
            int iBest = 2;
            char* pRes = const_cast<char*>( "640x480x8" );
            if ( m_iCpuSpeed > 100 )  // 800x600
            {
                iBest = 3;
                pRes = const_cast<char*>( "800x600x8" );
                dev.dmPelsWidth = 800;
                dev.dmPelsHeight = 600;
                if ( m_iCpuSpeed > 130 ) // 1024x768
                {
                    iBest = 4;
                    pRes = const_cast<char*>( "1024x768x8" );
...
// check the version
    ...
    try {
        ...
        // must replace m_hPrevInstance with mutex

        m_hMutex = OpenMutex( MUTEX_ALL_ACCESS, FALSE, _T("ENATIONS_INSTANCE"));  // request full access, handle not inheritable, object name

        // RedText class for -#s in dialogs
        if ( m_hMutex == NULL ) { // was m_hPrevInstance, but that has been removed entirely.
            WNDCLASS wc;
            memset( &wc, 0, sizeof( wc ) );
        ...
        m_hMutex = CreateMutex( NULL, FALSE, _T("ENATIONS_INSTANCE"));  // default security descriptor, mutex not owned, object name
        if ( m_hMutex == NULL ) {
            if ( GetLastError() == ERROR_ALREADY_EXISTS ) {
                Log( "Mutex for checking previous instance already exists, but failed to open using OpenMutex. Is another instance running?" );
                ThrowError(ERR_APP_START);
            }
        }
    ...
    if ( theMusicPlayer._GetHDig() == 0 )
        ...
        switch ( iFmt ) {
        case DIG_F_MONO_8:
            psFmt = const_cast<char*>( "K/8-bit/Mono, " );
            break;
        case DIG_F_MONO_16:
            psFmt = const_cast<char*>( "K/16-bit/Mono, " );
            break;
        case DIG_F_STEREO_8:
            psFmt = const_cast<char*>( "K/8-bit/Stereo, " );
            break;
        case DIG_F_STEREO_16:
            psFmt = const_cast<char*>( "K/16-bit/Stereo, " );
            break;
        default:
            psFmt = const_cast<char*>( "" );
        }
...
void CConquerApp::AssertValid() const {
    ...
    if ( m_pdlgMain != nullptr ) {
        ASSERT_VALID( m_pdlgMain );
    }
    if ( m_pCreateGame != nullptr ) {
        ASSERT_VALID( m_pCreateGame );
    }
}
#endif
...
void CDlgStackDump::OnCopyStack() {
    ...
    _itoa( m_pe->m_uEc, sNum1, 16 );
    _itoa( (int)m_pe->m_pExCode, sNum2, 16 );
    CString sMsg;
    if ( ms.dwAvailPageFile / ONE_MEG < 8 ) {
        sMsg.LoadString( IDS_OUT_OF_MEMORY );
        sMsg += "\r\n";
    ...
    for ( int iOn = 0; iOn < NUM_EXCEP; iOn++ ) {
        _itoa( m_pe->m_stack[iOn], sNum1, 16 );
