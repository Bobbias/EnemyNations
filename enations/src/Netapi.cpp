...
#include "wind22\include\logging.h" // dave's logging system NOTE: Added for logPrintf function.
...
static void CmdPlayerTaken( CNetSelectPlyr* pMsg ) {
    ...
    if ( ( theApp.m_pCreateGame != NULL ) ||
         //( ( (CJoinMulti*)theApp.m_pCreateGame )->m_dlgPickPlayer == nullptr) ) || // NOTE: commented this out for now. I'm not sure if this hurts stability or not, but I'm not sure how to correctly implement this check.
         ( ( (CJoinMulti*)theApp.m_pCreateGame )->m_dlgPickPlayer.m_hWnd == NULL ) )
        return;
    TRAP();
...
#ifdef _LOGOUT // NOTE: changed from _DEBUG. logPrintf is only implemented if _LOGOUT is defined!
