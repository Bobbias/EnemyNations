...
void CWndArea::Create( CMapLoc const& ml, CUnit* pUnit, BOOL bFirst ) {
    ...
    if ( CreateEx( 0, sWndCls, sTitle, dwStyle, rect.left, rect.top, rect.Width(), rect.Height(), theApp.m_pMainWnd->m_hWnd, NULL, NULL ) == 0 )
...
void CWndArea::OnPaint() {
    ...
    if ( !rectDIB.IsRectEmpty() ) {
        // Render the rect
        theMap.UpdateRect( m_aa, rectDIB, CDrawParms::UPDATE_MODE::DRAW );
...
void CWndArea::OnLButtonUp( UINT nFlags, CPoint point ) {
    ...
    case normal_select:
    {
        ...
        if ( ( abs( point.x - m_ptLMB.x ) >= theMap.HexWid( m_aa.m_iZoom ) / 2 ) ||
            ...
            int iNumInBox = 0;
            CVehicle* pVehLastInBox = {}; // NOTE: Added initialization. Compiler complains at line 2010 without this.
            while ( pos != NULL ) {
                DWORD dwID;
                CVehicle* pVeh;
