...
class CTerrain;
class CVehicle;
enum class CMaterialTypes::TYPE: int;
...
public:
    enum class TERRAIN_TYPE {   // terrain types
    ...
    enum class ROAD_FACING {   // road facings
        R_HORZ,
        R_VERT,
        ...
        R_X,
        R_PATH
    }; // show when have road cursor

    enum class COASTLINE {   // coastlines
        LAND_UL, // 1/4 land
        LAND_UR,
        LAND_LR,
        LAND_LL,
    ...
    enum {
        ul = 0x01, ur = 0x02, ll = 0x04, lr = 0x08, bldg = 0x10,
...
protected:
    ...
    int   m_iWheelMult[static_cast<std::underlying_type_t<WHEEL_TYPES>>(WHEEL_TYPES::NUM_WHEEL_TYPES)]; // multiplier for movement
    int   m_iDefenseMult[static_cast<std::underlying_type_t<WHEEL_TYPES>>(WHEEL_TYPES::NUM_WHEEL_TYPES)]; // multiplier for defense at that location
...
protected:
    CTerrainData   m_Data[static_cast<std::underlying_type_t<CHex::TERRAIN_TYPE>>(CHex::TERRAIN_TYPE::NUM_TYPES)];
...
public:
    ...
    CHexValidMatrix* GetHexValidMatrix() {
        ASSERT( m_ptrhexvalidmatrix.Value() );
        return m_ptrhexvalidmatrix.Value();
    }

    CHexValidMatrix const* GetHexValidMatrix() const {
        ASSERT( m_ptrhexvalidmatrix.Value() );
        return m_ptrhexvalidmatrix.Value();
    }
