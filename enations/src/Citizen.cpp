...
void CPlayer::CitizenConstruction() {
    ...
    // do we need both?
    BOOL bAptOk = TRUE;
    if ( ( m_iAptCap < m_iPplBldg + m_iPplVeh ) && ( m_iOfcCap < m_iPplBldg ) ) // NOTE: m_iPplHave to m_iPplBldg
        bAptOk = RandNum( 1 );
    ...
    if ( ( bAptOk ) && ( m_iAptCap < m_iPplBldg + m_iPplVeh ) ) {
        ...
        if ( m_iPplBldg + m_iPplVeh < 5 * PPL_MUL )
            ...
            if ( m_iPplBldg + m_iPplVeh < 15 * PPL_MUL )
...
#if 0 // NOTE: Temporarily remove the offending code.
