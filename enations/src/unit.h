...
public:
    ...
    enum UNIT_TYPE {   // run time type identification - use GetUnitType ()
        ...
        CTurret *   GetTurret() {
            if ( m_ptrturret.Value() != nullptr ) {
                ASSERT_VALID( m_ptrturret.Value() );
            }
...
protected:
        ...
        LONG     m_aiStore [static_cast<std::underlying_type_t<CMaterialTypes::TYPE>>(CMaterialTypes::TYPE::NUM_TYPES)]; // materials on hand
        LONG     m_aiBeforeUpdate[static_cast<std::underlying_type_t<CMaterialTypes::TYPE>>(CMaterialTypes::TYPE::NUM_TYPES)]; // change from for net message
