...
#include <wind22\include\dibwnd.h>
#include "ourlog.h"
#include <type_traits>
...
public:
    ...
    enum class HexViewCoordJogDirection { LEFT, RIGHT };

    CHexCoord( int x, int y ) { m_iX = x; m_iY = y; }
    CHexCoord( CSubHex const& sub ); // exists in terrain.inl
    CHexCoord( const CViewHexCoord& viewhexcoord, HexViewCoordJogDirection jog_direction );
    CHexCoord( CHexCoord const& src ) { m_iX = src.m_iX; m_iY = src.m_iY; }
    CHexCoord( CMapLoc const& pt );
    CHexCoord( unsigned long src ) { // FIXME: should be explicit
        m_iX = src >> 16;
        m_iY = src & 0xFFFF;
    } // added for terrain.cpp lines 4121 and 4128
...
public:
    ...
    BOOL  IsHexHit( CPoint, const CRect & = CRect() )  const;
...
public:
    ...
    enum class TURN_DIR { TURN_CLOCKWISE, TURN_COUNTERCLOCKWISE };
    enum class ZOOM_LEVEL { ZOOM_IN, ZOOM_OUT };
    ...
    enum class SET_CENTER_METHOD { SET_CENTER_SCROLL, SET_CENTER_NOP, SET_CENTER_INVALIDATE };
    ...
    BOOL           IsHexHit( const CHexCoord&, CPoint, CRect& ) const; // NOTE: removed const from rrectBound. Original was CRect const& rrectBound
...
enum class WHEEL_TYPES { WALK = 0, WHEEL = 1, TRACK = 2, HOVER = 3, WATER = 4, NUM_WHEEL_TYPES };
...
public:
    ...
    static std::underlying_type_t<TYPE> GetNumTypes() { return ( static_cast<std::underlying_type_t<TYPE>>( TYPE::NUM_TYPES ) ); }
    static std::underlying_type_t<TYPE> GetNumBuildTypes() { return ( static_cast<std::underlying_type_t<TYPE>>( TYPE::NUM_BUILD_TYPES ) ); }

    static CString const& GetDesc( CMaterialTypes::TYPE iInd ) {
        return ( m_saDesc[static_cast<std::underlying_type_t<TYPE>>( iInd )] );
    }
    static COLORREF       GetRGB( CMaterialTypes::TYPE iInd ) {
        return ( m_rgb[static_cast<std::underlying_type_t<TYPE>>( iInd )] );
    }

    // to make CMinerals::CArchive& operator>> work
    friend CArchive& operator>> ( CArchive& ar, CMaterialTypes::TYPE& mt ) {
        ar >> mt;
        return ar;
    }

protected:
    static CString m_saDesc[static_cast<std::underlying_type_t<TYPE>>( TYPE::NUM_TYPES )];
    static COLORREF m_rgb[static_cast<std::underlying_type_t<TYPE>>( TYPE::NUM_TYPES ) + 1];
};
