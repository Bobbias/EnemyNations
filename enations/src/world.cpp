...
void CWndWorld::_NewDir() {
    ...
    case 4:
        ...
        break;
    default:
        fnSetPixel = nullptr; // NOTE: added default to ensure fnSetPixel always has a value, and the switch statement has a default. This should never happen.
    }
    ...
    for ( int y = 0; y < m_cy; y++ ) {
        ...
        GotClr:
            if ( fnSetPixel != nullptr ) { // NOTE: Don't call fnSetPixel if it's somehow nullptr.
...
void CWndWorld::ReRender() {
    ...
    for ( int y = 0; y < m_cy; y++ ) {
        ...
        for ( ; x < iMax; x++ ) {
            ...
            CHex* pHex = theMap._GetHex( _hex );
            DWORD dwClr;
            CBuilding* pBldg = nullptr; // NOTE: initialized to nullptr to avoid warnings
            ...
            if ( pHex->GetUnits() & CHex::bldg ) {
                ...
                if ( pBldg->GetOwner()->IsMe() ) {
                    ...
                    if ( m_bBldgHit && fnSetPixel != nullptr ) { // NOTE: Don't call fnSetPixel if it's somehow nullptr.
            ...
            NotMinerals:
                ...
                if ( bdwVis & ( !pHex->GetVisible() ) ) {
                    ...
                    if ( bOkDraw && ( ( x + y ) & 1 ) && fnSetPixel != nullptr ) // NOTE: Don't call fnSetPixel if it's somehow nullptr.
