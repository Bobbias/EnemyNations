...
void CRsrchItem::AssertValid() const {
    ...
    for ( int iOn = 0; iOn < m_iNumBldgsRequired; iOn++ )
        ...
        for ( int iOn = 0; iOn < theRsrch.GetSize(); iOn++ ) {
            ...
            if ( !theGame.GetMe()->GetRsrch( iOn ).m_bDiscovered ) {
                ...
                if ( theGame.GetServerNetNum() == 0 )
                    if ( ( iOn == CRsrchArray::mail ) || ( iOn == CRsrchArray::email ) || ( iOn == CRsrchArray::telephone ) )
                        goto skip_it;

                // no xilitium
                if ( ( iOn == CRsrchArray::copper ) && ( ( theApp.IsShareware() ) || ( ( theApp.IsSecondDisk() ) && ( !theGame.IsNetGame() ) ) ) )
                    goto skip_it;
