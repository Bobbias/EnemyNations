...
BOOL CBuilding::InitRocket( CHexCoord const& hex, BOOL bMe ) {
    ...
    if ( bMe ) {
        ...
        if ( theApp.GetProfileInt( "Cheat", "AllUnits", 0 ) ) {
            int i = 0; // NOTE: This is an ugly hack, line 2889 depends on i existing outside the for loop scope
...
CVehicle* CVehicle::Create( const CSubHex& ptHead, const CSubHex& ptTail, int iVeh, int iOwner, DWORD ID, VEH_MODE iRouteMode, CHexCoord const& hexDest, DWORD dwIDBldg, int iDelay ) {
