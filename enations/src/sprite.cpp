...
#include <wind22\include\scanlist.h>
...
CDrawParms::UPDATE_MODE CDrawParms::g_uUpdateFlags;
...
CSpriteDIB::CalcBoundingRect() const {
    switch ( m_iType ) {
    case CSprite::SPRITE_TYPES::EFFECT:
    case CSprite::SPRITE_TYPES::STRUCTURE: return StructureCalcBoundingRect();
    case CSprite::SPRITE_TYPES::TERRAIN:  return TerrainCalcBoundingRect();
    case CSprite::SPRITE_TYPES::VEHICLE:  return VehicleCalcBoundingRect();
...
// CSpriteDIB::IsHit
//--------------------------------------------------------------------------
BOOL CSpriteDIB::IsHit( CPoint const& ptCursor ) const {
    switch ( m_iType ) {
    case CSprite::SPRITE_TYPES::EFFECT:
    case CSprite::SPRITE_TYPES::STRUCTURE: return StructureIsHit( ptCursor );
    case CSprite::SPRITE_TYPES::TERRAIN:  return TerrainIsHit( ptCursor );
    case CSprite::SPRITE_TYPES::VEHICLE:  return VehicleIsHit( ptCursor );
    }
...
CSpriteDIB::TerrainIsHit(
    ...
    CHexCoord hexcoord( TerrainGetParms()->m_hexcoord );
    auto zeros = CRect( 0, 0, 0, 0 ); // NOTE: instantiate before passing to function. Original function call instantiated this CRect directly in the call.
    return xpanimatr->IsHexHit( hexcoord, ptCursor, zeros );
}
...
CSpriteDIB::Draw() const {
    ...
    if ( !Rect().IsRectEmpty() )
        switch ( m_iType ) {
        case CSprite::SPRITE_TYPES::EFFECT:
        case CSprite::SPRITE_TYPES::STRUCTURE: return StructureDraw();
        case CSprite::SPRITE_TYPES::TERRAIN:  return TerrainDraw();
        case CSprite::SPRITE_TYPES::VEHICLE:  return VehicleDraw();
...
CSpriteDIB::TerrainDraw() const {
    ...
    CRect rectBound( aptHex[0].x, Min( Min( aptHex[0].y, aptHex[1].y ), aptHex[2].y ),
                     aptHex[2].x, Max( Max( aptHex[0].y, aptHex[2].y ), aptHex[3].y ) );
...
//--------------------------------------------------------------------------
// Bounding rect in window client coords
CRect CSpriteDIB::StructureDrawToDIB( CDIB* pdib, CPoint const& ptDst, CRect  const& rectClip /* Clipping rect in window client coords */) const {
...
//-----------------------------------------------------------------------
void CSpriteDIB::TerrainDrawQuad( int aiShadeIndex[2], CPoint aptHex[4], BOOL bInvisible, BOOL bHatch, int iColorIndex ) const {
    ...
    auto feather_left = static_cast<std::underlying_type_t<CTerrainDrawParms::FEATHER_DIR>>(CTerrainDrawParms::FEATHER_DIR::FEATHER_TL);
    for ( i = 0; i < 2; ++i ) {
        switch ( TerrainGetParms()->m_aeFeather[ feather_left + i] ) {
        case CTerrainDrawParms::FEATHER_TYPE::FEATHER_INOUT:
        ...
        case CTerrainDrawParms::FEATHER_TYPE::FEATHER_IN:
        ...
        case CTerrainDrawParms::FEATHER_TYPE::FEATHER_OUT:
        ...
        case CTerrainDrawParms::FEATHER_TYPE::FEATHER_NONE:
    ...
    auto feather_right = static_cast<std::underlying_type_t<CTerrainDrawParms::FEATHER_DIR>>(CTerrainDrawParms::FEATHER_DIR::FEATHER_TR);
    for ( i = 0; i < 2; ++i ) {
        switch ( TerrainGetParms()->m_aeFeather[feather_right + i] ) {
        case CTerrainDrawParms::FEATHER_TYPE::FEATHER_INOUT:
...
//-----------------------------------------------------------------------
void CSpriteDIB::TerrainDrawQuadVertHatched( int aiShadeIndex[2], CPoint aptHex[4], BOOL bInvisible, int iColorIndex ) const {
    int iDelY = aptHex[3].y - aptHex[1].y;
...
// CSpriteDIB::VehicleDraw
//--------------------------------------------------------------------------
CRect CSpriteDIB::VehicleDraw( CPoint const aptVertex[4] ) const {
    CDIB* pdib = xpdibwnd->GetDIB();
    ...
    for ( iY = iTopY; iY <= iBottomY; ++iY ) {
        int fixX = Max( (int)( prVp->left << 16 ) + 0x00008000, *pfixLeft++ ); // NOTE: Added int cast.
        int iX = fixX >> 16;
        ...
        int fixRight = Min( *pfixRight++, (int)( prVp->right << 16 ) + 0x00008000 ); // NOTE: Added int cast.
...
void CUnitTile::Serialize( CArchive& ar ) {
    ...
    auto anim_count = static_cast<std::underlying_type_t<CSpriteView::ANIM_TYPE>>(CSpriteView::ANIM_TYPE::ANIM_COUNT);

    if ( ar.IsStoring() ) {
        ar << m_bDamage;
        ar << static_cast<BYTE>(m_eLayer);
        ar << m_maploc;
        ...
        if ( m_psprite ) {
            ar << static_cast<BYTE>( m_psprite->GetID() );
            ar << static_cast<BYTE>( m_psprite->GetIndex() );
        } else {
...
//-------------------------------------------------------------------------
CUnitTile::CUnitTile():
    CTile( CTile::TILE_TYPE::UNIT_TILE ),
    m_eLayer( CStructureSprite::LAYER_TYPE::FOREGROUND_LAYER ),
    m_maploc( 0, 0 ),
    m_bDamage( 0 )
{
    auto anim_count = static_cast<std::underlying_type_t<CSpriteView::ANIM_TYPE>>(CSpriteView::ANIM_TYPE::ANIM_COUNT);
...
//-------------------------------------------------------------------------
void CUnitTile::EnableAnimations( BOOL bEnable ) {
    auto anim_count = static_cast<std::underlying_type_t<CSpriteView::ANIM_TYPE>>(CSpriteView::ANIM_TYPE::ANIM_COUNT);
    for ( int i = 0; i < anim_count; ++i )
...
//-------------------------------------------------------------------------
void CUnitTile::PauseAnimations( BOOL bPause ) {
    auto anim_count = static_cast<std::underlying_type_t<CSpriteView::ANIM_TYPE>>(CSpriteView::ANIM_TYPE::ANIM_COUNT);
    for ( int i = 0; i < anim_count; ++i )
...
//-------------------------------------------------------------------------
void CUnitTile::ResetAnimations() {
    auto anim_count = static_cast<std::underlying_type_t<CSpriteView::ANIM_TYPE>>(CSpriteView::ANIM_TYPE::ANIM_COUNT);
    for ( int i = 0; i < anim_count; ++i )
        m_aambient[i].Reset();
}
...
//-------------------------------------------------------------------------
void CUnitTile::SetOneShotAnimations( BOOL bOneShot ) {
    auto anim_count = static_cast<std::underlying_type_t<CSpriteView::ANIM_TYPE>>(CSpriteView::ANIM_TYPE::ANIM_COUNT);
    for ( int i = 0; i < anim_count; ++i )
...
//-------------------------------------------------------------------------
CSimpleTile::CSimpleTile():
    CTile( CTile::TILE_TYPE::SIMPLE_TILE )
{}

//-------------------------------------------------------------------------
// CSpriteView::GetBase
//-------------------------------------------------------------------------
CSpriteDIB const* CSpriteView::GetBase( int iIndex ) const {
...
//-------------------------------------------------------------------------
CSpriteDIB const* CSpriteView::GetOverlay( int iIndex ) const {
...
// CSpriteView::GetAnim
//-------------------------------------------------------------------------
CSpriteDIB const* CSpriteView::GetAnim( CSpriteView::ANIM_TYPE eAnim, int iIndex ) const {
    auto anim = static_cast<std::underlying_type_t<CSpriteView::ANIM_TYPE>>( eAnim );
    return m_apspritedibAnim[anim] + iIndex;
}

//-------------------------------------------------------------------------
// CSpriteView::GetHotSpotPoint
//-------------------------------------------------------------------------
CPoint CSpriteView::GetHotSpotPoint( CViewCoord const& hotspot, int iZoom ) const {
...
//---------------------------------------------------------------------------
BOOL CSpriteView::IsHitClip( CDrawParms& drawparms, CPoint const& ptCursor, CRect* prectClip ) const {
    ASSERT_STRICT_VALID( this );
    ...
    for ( int j = 0; j < anim_count; ++j ) {
        ...
        if ( CTile::TILE_TYPE::UNIT_TILE == ptile->GetTileType() )
            bCheckLayer = ( (CUnitTile*)ptile )->GetAmbient( static_cast<CSpriteView::ANIM_TYPE>(j) )->IsEnabled();

        if ( bCheckLayer )
            for ( int i = 0; i < AnimCount( ANIM_TYPE( j ) ); ++i ) {
...
// CSpriteView::IsHit
//---------------------------------------------------------------------------
BOOL CSpriteView::IsHit( CDrawParms& drawparms, CPoint const& ptCursor ) const {
...
// CSpriteView::Draw
//---------------------------------------------------------------------------
// Bounding rect for bitmaps drawn
CRect CSpriteView::Draw( CDrawParms& drawparms ) {
    CRect rect = CDrawParms::GetClipRect();
...
//---------------------------------------------------------------------------
// Bounding rect for bitmaps drawn
CRect CSpriteView::_Draw( CDrawParms& drawparms ) {
    ASSERT_STRICT_VALID( this );
    ...
    if ( 0 < AnimCount( CSpriteView::ANIM_TYPE::ANIM_BACK_1 ) ) {
        ASSERT_STRICT( CTile::UNIT_TILE == CSpriteDIB::GetParms()->m_ptile->GetTileType() );
        rectBound |= DrawUnitAnimation( CSpriteView::ANIM_TYPE::ANIM_BACK_1 );
    }

    if ( 0 < AnimCount( CSpriteView::ANIM_TYPE::ANIM_BACK_2 ) ) {
        ASSERT_STRICT( CTile::UNIT_TILE == CSpriteDIB::GetParms()->m_ptile->GetTileType() );
        rectBound |= DrawUnitAnimation( CSpriteView::ANIM_TYPE::ANIM_BACK_2 );
    }
    ...
    if ( AnimCount( CSpriteView::ANIM_TYPE::ANIM_FRONT_1 ) > 0 )
        if ( CSpriteDIB::GetParms()->m_ptile->GetTileType() == CTile::TILE_TYPE::UNIT_TILE)
            rectBound |= DrawUnitAnimation( CSpriteView::ANIM_TYPE::ANIM_FRONT_1 );
        else
            rectBound |= DrawSimpleAnimation();

    if ( AnimCount( CSpriteView::ANIM_TYPE::ANIM_FRONT_2 ) > 0 ) {
        ASSERT_STRICT( CTile::UNIT_TILE == CSpriteDIB::GetParms()->m_ptile->GetTileType() );
        rectBound |= DrawUnitAnimation( CSpriteView::ANIM_TYPE::ANIM_FRONT_2 );
    }
...
//---------------------------------------------------------------------------
void CSpriteView::DrawSelected( CRect const& rect, int iColor ) {
    CDIB* pdib = xpdibwnd->GetDIB();
...
//---------------------------------------------------------------------------
CRect CSpriteView::DrawUnitAnimation( ANIM_TYPE  eAnim ) {
    CUnitTile*        punittile = static_cast<CUnitTile*>( CSpriteDIB::GetParms()->m_ptile );
...
CRect
CSpriteView::DrawSimpleAnimation() {
    ANIM_TYPE         eAnim      = CSpriteView::ANIM_TYPE::ANIM_FRONT_1;
    ...
    if ( CDrawParms::IsInvalidateMode() && ( iHoldLeft == 0 ) ) {
...
//-------------------------------------------------------------------------
CSpriteView* CSprite::GetSpriteView( int iIndex, BOOL bCheckValid ) const {
    CSpriteView* pspriteview = (CSpriteView*)( (BYTE*)m_ptrspritehdr.Value() + m_piViewOffsets[iIndex] ); // NOTE: DANGER WTF are they doing here?
...
//-------------------------------------------------------------------------
BYTE const* CSprite::GetDIBPixels( int iSuperviewIndex, CDIBLayoutInfo const& diblayoutinfo ) {
#ifdef _DEBUG
...
//---------------------------------------------------------------------------
// pbySrc - 24 bpp
void CSprite::ColorConvert( BYTE* pbyDst, BYTE const* pbySrc, long lLenDst, long lLenSrc, int iZoom, int iSuperviewIndex ) {
    BOOL bCompressed = ( m_iType != CSprite::SPRITE_TYPES::TERRAIN ) && ( m_iType != CSprite::SPRITE_TYPES::VEHICLE );
...
//-------------------------------------------------------------------------
CSpriteView* CTerrainSprite::GetView( int iDir, int iDamage ) {
...
//-------------------------------------------------------------------------
CSpriteView* CStructureSprite::GetView( int iDir, int iLayer, int iStage, int iDamage ) {
...
//-------------------------------------------------------------------------
CSpriteView* CVehicleSprite::GetView( int iDir, int iTilt, int iDamage ) {
...
//-------------------------------------------------------------------------
CSpriteCollection::CSpriteCollection( const char* pszRif ):
    m_pptrsprite( NULL ),
    m_nSprite( 0 ),
    m_bOpen( FALSE ),
    m_strRif( pszRif )
...
//-------------------------------------------------------------------------
CSprite const* CSpriteCollection::GetSprite( int iID, int iIndex, BOOL bStrict ) const {
...
// CSpriteCollection::GetSprite
//-------------------------------------------------------------------------
CSprite* CSpriteCollection::GetSprite( int iID, int iIndex, BOOL bStrict ) {
    ASSERT_VALID( this );
    ASSERT( m_bOpen );
...
//-------------------------------------------------------------------------
int CSpriteCollection::GetCount( int  iID ) const {
    ASSERT_VALID( this );
...
//-------------------------------------------------------------------------
CFile* CSpriteCollection::GetFile() {
    if ( !m_ptrfile.Value() ) {
...
//--------------------------------------------------------------------------
int CSpriteCollectionInfo::GetCount( int iID ) const {
    iID = MapID( iID, FALSE );
...
//--------------------------------------------------------------------------
int CSpriteCollectionInfo::GetIndex( int iID, int iOffset, BOOL bStrict ) const {
...
//-------------------------------------------------------------------------
int CSpriteCollectionInfo::MapID( int iID, BOOL bStrict ) const {
    ASSERT_VALID( this );
...
//-------------------------------------------------------------------------
int CSpriteCollectionInfo::MapOffset( int iID, int iOffset, BOOL bStrict ) const {
    ASSERT_STRICT_VALID( this );

    ASSERT( iID <= m_iMaxID );

    if ( m_pspriteidinfo[iID].GetCount() <= iOffset ) {
        if ( bStrict || m_pspriteidinfo[iID].GetCount() == 0) {
            return -1;
        } else {
            return 0;
        }
