...
public:
    ...
    enum TRANS_TYPE {
        construction,  // this is returned by GetType ()
        med_truck,     // obsolete
        heavy_truck,
        light_scout,
        med_scout,
    ...
    enum TRANS_FLAGS {
        FLconstruction = 0x01, FLboat = 0x02, FLcarrier = 0x04,
...
public:
    ...
    CRoute() {}
    CRoute( CHexCoord& hex, int iType ) {
        ASSERT( ( 0 <= iType ) && ( iType <= 2 ) );
...
class CVehicle: public CUnit {
    friend const CMsgVehCompLocElem CMsgVehCompLocElem::operator= ( CVehicle const& src ); // NOTE: added const marking
    ...
    friend class CMsgVehCompLocElem; // Added to make CMsgVehCompLocElem::operator= stop complaining about inaccessible members of CVehicle

public:
    enum VEH_MODE {
        stop,    // m_iMode - vehicle is stopped
    ...
    static CVehicle* Create( const CSubHex& ptHead, const CSubHex& ptTail,
                             ...
                             CHexCoord const& hexDest = CHexCoord( 0, 0 ), DWORD dwIDBldg = 0, int iDelay = 0 );
...
protected:
    ...
    enum {
        do_spotting = 0x01, new_loc = 0x02, hp_controls = 0x04, told_ai_stop = 0x08,
