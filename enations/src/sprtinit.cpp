...
void CEffect::InitSprites() {
    ...
    for ( int i = 0; i < GetCount( smoke ); ++i ) {
    ...
    int  iTypeOverride ) // type override
{
    ...
    BOOL bReadThread; // NOTE: Refactored for clarity. original was `BOOL bReadThread = W32s != iWinType;`
    if ( iWinType == W32s ) { // If iWinType == W32s do not read thread
        bReadThread = false;
    } else {
        bReadThread = true;
    }
    HANDLE hThread = nullptr; // NOTE: Added initialization to nullptr to avoid warning about potentially uninitialized variable.
    ...
    if ( 0 < i ) // fixme: relies on above i leaking from for loop
        m_ptrspritecollectioninfo->SetInfo( iPrevID, iStartIndex, iIndex + 1 );
