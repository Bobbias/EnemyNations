...
CDlgJoinGame::CDlgJoinGame( CWnd* pParent /*=NULL*/ )
    ...
    (void)m_sVer.LoadString( IDS_EN_VER );
    m_sVer += VER_STRING;
    CString temp;
    (void)temp.LoadString( IDS_VP_VER );
    long lVer = CNetApi::GetVersion();
    m_sVpVer.Format( "%s%d.%d.%d", temp, HIBYTE(HIWORD(lVer)), LOBYTE(HIWORD(lVer)), LOWORD(lVer) );
...
void CDlgJoinGame::OnSessionEnum( LPCVPSESSIONINFO pSi ) {
    ...
    if ( ( *pPw != 0 ) && ( _stricmp( pPw, m_pJm->m_dlgJoinPublish.m_strPw ) ) )
        return;
