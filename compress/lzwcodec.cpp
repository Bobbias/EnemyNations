...
{
    int tableIndex;
    for ( tableIndex = 0; tableIndex < iTABLE_SIZE; tableIndex++ ) dictionary[tableIndex].iCodeValue = iUNUSED;
...
void* LZWCoDec::Compress( const void* pUncompressedBuffer, int iUncompressedBufSize, void* pCompressedBuffer,
...
void* LZWCoDec::Decompress( const void* pCompressedBuffer, int iCompressedBufSize, void* pUncompressedBuf,
    ...
    for ( ;; )
    {
        ...
        *pBufPos++ = (BYTE)oldCode;
        ...
        for ( ;; )
        {
            ...
            while ( count > 0 ) *pBufPos++ = decodeStack[--count];
...
DWORD LZWCoDec::FindChildNode( int parentCode, int childCharacter )
{
    ...
    for ( ;; )
    {
        if ( dictionary[index].iCodeValue == iUNUSED )
            return index;
        if ( dictionary[index].iParentCode == parentCode && dictionary[index].cChar == (char)childCharacter )
            return index;
        if ( index >= offset )
            index -= offset;
        else
